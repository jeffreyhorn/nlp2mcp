...
stat_x(n,k).. 0.5 * (sum(np__, (x(np__,k) - xtilde(np__,k)) * w(n,np__,k)) + sum(n__, (x(n__,k) - xtilde(n__,k)) * w(n__,n,k))) + ((-1) * a(n,n)) * nu_stateq(n,k) + sum(np, (((-1) * a(np+1,n)) * nu_stateq(n+1,k))$(ord(n) <= card(n) - 1)) + nu_stateq(n,k-1)$(ord(k) > 1) + sum(np, (((-1) * a(np-1,n)) * nu_stateq(n-1,k))$(ord(n) > 1)) =E= 0;
