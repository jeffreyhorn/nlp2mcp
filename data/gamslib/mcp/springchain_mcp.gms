...
stat_delta_x(n).. (nu_delta_x_eq(n) + (2 * delta_x(n) * lam_link_up(n))$(ord(n) > 1))$(ord(n) > 1) =E= 0;
stat_delta_y(n).. (nu_delta_y_eq(n) + (2 * delta_y(n) * lam_link_up(n))$(ord(n) > 1))$(ord(n) > 1) =E= 0;
...
stat_x(n).. (((-1) * nu_delta_x_eq(n)) + nu_delta_x_eq(n+1)$(ord(n) <= card(n) - 1) + nu_x_fx_n0$(sameas(n, 'n0')) + nu_x_fx_n10$(sameas(n, 'n10')))$(ord(n) > 1 or ord(n) <= card(n) - 1 or sameas(n, 'n0') or sameas(n, 'n10')) =E= 0;
...
x.fx(n)$(not ((ord(n) > 1) or (sameas(n, 'n0')) or (sameas(n, 'n10')))) = 0;
