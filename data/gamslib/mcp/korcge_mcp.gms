...
stat_p(i).. x(i) * nu_absorption(i) + sum(j, (-1) * io(i,j) * nu_actp(j)) + sum(j, (-1) * imat(i,j) * nu_pkdef(j)) + ((-1) * pwts(i)) * nu_pindexdef + cd(i) * nu_cdeq(i) + ((-1) * gd(i)) * nu_gruse + kio(i) * dst(i) * nu_prodinv(i) + (kio(i+1) * dst(i) * nu_prodinv(i+1))$(ord(i) <= card(i) - 1) + (kio(i+2) * dst(i) * nu_prodinv(i+2))$(ord(i) <= card(i) - 2) + (kio(i-1) * dst(i) * nu_prodinv(i-1))$(ord(i) > 1) + (kio(i-2) * dst(i) * nu_prodinv(i-2))$(ord(i) > 2) - piL_p(i) =E= 0;
