...
stat_pq(i).. q(i) * nu_valueq(i) + sum(j, (-1) * (1 + tnd(i)) * a(i,j) * nu_pnddet(j)) + ((-1) * (1 + tfd(i) + trmd(i))) * nu_pcdet(i) + (((-1) * (exscale * aex(i) * (er00 * pie(i) / (pq(i) * (1 + trmx(i)))) ** eta(i) * eta(i) / (er00 * pie(i) / (pq(i) * (1 + trmx(i)))) * ((-1) * (er00 * pie(i) * (1 + trmx(i)))) / sqr(pq(i) * (1 + trmx(i))))) * nu_export(i))$(ie(i)) + ((-1) * (fd(i) * trmd(i) + ex(i) * trmx(i))) * nu_margdet + ex(i) * (1 + trmx(i)) * nu_fbudget + ((-1) * dst(i)) * nu_invsav - piL_pq(i) =E= 0;
