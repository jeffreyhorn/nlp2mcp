...
stat_p0.. ((-1) * (pump_efficiency * q / sqr(pump_efficiency))) * nu_pumping_energy + ((-1) * (tmp2 * 2 * pi / 4)) * nu_load_capacity + pi * h ** 3 * nu_inlet_pressure + ((-1) * (0.001 * g * sqr(2 * pi * r * h))) * lam_limit1 - piL_p0 + piU_p0 =E= 0;
stat_q.. ((-1) * (pump_efficiency * (p0 - P1) / sqr(pump_efficiency))) * nu_pumping_energy + 9336 * delta_t * C * gamma * nu_temp_rise + ((-1) * (tmp1 * 6 * mu)) * nu_inlet_pressure + 2 * gamma * q * lam_limit1 - piL_q + piU_q =E= 0;
