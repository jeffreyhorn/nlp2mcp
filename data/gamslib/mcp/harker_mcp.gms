...
stat_d(n).. nu_bal + sum(l, nu_dbal(l)) + sum(l, nu_in(l)) + sum(l, ((-1) * (coefs(n,"rho") - pm * coefs(n,"eta") * sum(lp, tt(l,lp)))) * nu_objoli) =E= 0;
stat_s(n).. ((-1) * nu_bal) + sum(l, nu_sbal(l)) + (coefs(n,"alpha") + 2 * coefs(n,"beta") * s(n)) * nu_objoli =E= 0;
