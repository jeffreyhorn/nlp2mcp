...
stat_rt(i).. ((-1) * taud) * nu_eqTd + ((-1) * ssp) * nu_eqSp + ((-1) * (pq(i) * alpha(i) / sqr(pq(i)))) * nu_eqXp(i) + (((-1) * (pq(i) * alpha(i) / sqr(pq(i)))) * nu_eqXp(i+1))$(ord(i) <= card(i) - 1) + (((-1) * (pq(i) * alpha(i) / sqr(pq(i)))) * nu_eqXp(i-1))$(ord(i) > 1) + nu_eqRT(i) =E= 0;
