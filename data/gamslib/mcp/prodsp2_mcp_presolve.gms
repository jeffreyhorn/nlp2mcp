...
$onMultiR
$include "data/gamslib/raw/prodsp2.gms"
$offMulti

* Transfer NLP duals to MCP multiplier initialization
lam_lbal2.l(j) = abs(lbal2.m(j));
lam_extra.l = abs(extra.m);

* Transfer variable marginals to bound multipliers
piL_x.l(i)$(abs(x.l(i) - x.lo(i)) < 1e-6) = abs(x.m(i));
piL_v2.l(j)$(abs(v2.l(j) - v2.lo(j)) < 1e-6) = abs(v2.m(j));

* Reset any NA/UNDF warm-start multiplier levels to 0 (#1322)
lam_lbal2.l(j)$(NOT (lam_lbal2.l(j) > -inf and lam_lbal2.l(j) < inf)) = 0;
lam_extra.l$(NOT (lam_extra.l > -inf and lam_extra.l < inf)) = 0;
piL_x.l(i)$(NOT (piL_x.l(i) > -inf and piL_x.l(i) < inf)) = 0;
piL_v2.l(j)$(NOT (piL_v2.l(j) > -inf and piL_v2.l(j) < inf)) = 0;
