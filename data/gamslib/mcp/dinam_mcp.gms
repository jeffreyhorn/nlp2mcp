...
Parameters
    ...
    sf(eit,s,l,s) /change.eng.'1'.eng 1, change.tech.'0'.eng 0.3, change.tech.'1'.tech 1, change.admin.'2'.eng -2, change.admin.'1'.eng 0.6, change.admin.'0'.eng 0.1, change.admin.'1'.tech -1.9, change.admin.'0'.tech 0.9, change.admin.'1'.admin 1, change.man.'0'.tech -2, change.man.'1'.admin 1, input.eng.'2'.eng -0.17, input.eng.'1'.eng -0.14, input.eng.'0'.eng -0.02, input.tech.'2'.eng -0.09, input.tech.'0'.eng -0.18, input.tech.'0'.tech -0.16, input.admin.'0'.tech -0.04/
...
stat_em(te).. sum(tep, ((-1) * 1$(ts(te,tep))) * nu_exdef(te)) + sum(tep, sum((i,t), es(i) * zmi(t,tep) * 1$(ts(t,tep)) * lam_mb(i,t))) - piL_em(te) =E= 0;
stat_fc(te)$(t(te)).. (-1) * nu_fgap(te) - piL_fc(te) =E= 0;
stat_fdp(te)$(t(te)).. sum(tep, (-1 + pci * 1$(ts2(te,tep))) * nu_fgap(te)) + lam_ffdp$(t(te)) - piL_fdp(te) =E= 0;
...
stat_ka(te).. sum(tep, sum((s,t), ((-1) * (alr(t) * interval * 1$(ts2(t,tep)))$(sun(s))) * nu_drql(s,t))) + sum(tep, sum(s, (((-1) * (interval * alr(te) * 1$(ts2(te,tep)))$(sun(s))) * nu_trql(s,te))$(ord(te) > last))) + nu_ginv(te)$(ord(te) <= last) + sum(j, sum(inc, bk(inc,j,te)) * nu_fexch(te)) + sum(j, sum((i,t), bk(i,j,t) * lam_mb(i,t))) - piL_ka(te) =E= 0;
