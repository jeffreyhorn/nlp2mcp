...
stat_em(te).. sum(tp, (((-1) * 1$(ts(te,tp))) * nu_exdef(te))$(t(te))) + sum(tep, sum((i,t), (es(i) * zmi(tep,te) * 1$(ts(tep,t)) * lam_mb(i,tep))$(t(tep)))) - piL_em(te) =E= 0;
...
stat_ka(te).. sum((s,t), (((-1) * (alr(te) * interval * 1$(ts2(te,t)))$(sun(s))) * nu_drql(s,te))$(t(te))) + sum(s, ((((-1) * (alr(te) * (1 + interval * 1$(ts2(te,te))))$(sun(s))) + (alr(te) * interval * 1$(ts2(te,te)))$(sun(s))) * nu_drql(te,s))$(t(s))) + sum(tep, sum(s, (((-1) * (interval * alr(te+7) * 1$(ts2(te+7,tep)))$(sun(s))) * nu_trql(s,te))$(ord(te) > last))) + nu_ginv(te)$(ord(te) <= last) + (sum(inc, bk(inc,"agri",te)) * nu_fexch(te))$(t(te)) + sum((i,t), (bk(i,"agri",t) * lam_mb(i,t))$(t(t))) - piL_ka(te) =E= 0;
