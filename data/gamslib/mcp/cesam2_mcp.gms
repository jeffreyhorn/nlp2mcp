...
stat_tsam(i,j).. (nu_ROWSUM(i)$(ii(i)) + nu_COLSUM(j)$(jj(j)) + (nu_SAMCOEF(i,j)$(ii(i) and jj(j)))$(NONZERO(i,j)) + (nu_TSAMEQ(i,j)$(ii(i) and jj(j)))$(IVAL(i,j)) + nu_GDPDEF$(sameas(i, 'ACT') and sameas(j, 'GOV')) + ((-1) * nu_GDPDEF)$(sameas(i, 'FAC') and sameas(j, 'ACT') or sameas(i, 'GOV') and sameas(j, 'ACT') or sameas(i, 'GOV') and sameas(j, 'COM')) + ((-1) * nu_GDPFCDEF)$(sameas(i, 'FAC') and sameas(j, 'ACT')))$(ii(i) and ii(j)) =E= 0;
...
stat_w3(i,j,jwt).. (((log(w3(i,j,jwt) + delta) - log(wbar3(i,j,jwt) + delta) + w3(i,j,jwt) * 1 / (w3(i,j,jwt) + delta)) * 1$(nonzero(i,j)))$(jwt3(jwt)) + (((((-1) * vbar3(i,j,jwt)) * nu_ERROR3EQ(i,j))$(ii(i) and jj(j)))$(NONZERO(i,j)))$(jwt3(jwt)) + ((nu_SUMW3(i,j)$(ii(i) and jj(j)))$(NONZERO(i,j)))$(jwt3(jwt)))$(ii(i) and ii(j) and jwt3(jwt)) =E= 0;
stat_y(i).. (nu_ROWSUMEQ(i)$(ii(i)) - nu_ROWSUM(i)$(ii(i)) - nu_COLSUM(i)$(jj(i)) + sum(ii__, ((((-1) * a(ii__,i)) * nu_SAMCOEF(ii__,i))$(ii(ii__) and jj(i)))$(NONZERO(ii__,i))))$(ii(i)) =E= 0;
