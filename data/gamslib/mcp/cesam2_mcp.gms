...
wbar1(i,jwt)$(NOT (wbar1(i,jwt) > -inf and wbar1(i,jwt) < inf)) = 0;
...
stat_tsam(i,j).. (nu_ROWSUM(i)$(ii(i)) + nu_COLSUM(j)$(jj(j)) + (nu_SAMCOEF(i,j)$(ii(i) and jj(j)))$(NONZERO(i,j)) + (nu_TSAMEQ(i,j)$(ii(i) and jj(j)))$(IVAL(i,j)) + nu_GDPDEF$(sameas(i, 'ACT') and sameas(j, 'GOV') or sameas(i, 'FAC') and sameas(j, 'ACT') or sameas(i, 'GOV') and sameas(j, 'ACT') or sameas(i, 'GOV') and sameas(j, 'COM')) + ((-1) * nu_GDPFCDEF)$(sameas(i, 'FAC') and sameas(j, 'ACT')))$(ii(i) and ii(j)) =E= 0;
