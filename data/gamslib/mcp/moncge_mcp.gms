...
Parameters
    SAM(u,v) /BRD.BRD 21, BRD.MLK 8, BRD.HOH 20, BRD.GOV 19, BRD.INV 16, BRD.EXT 8, MLK.BRD 17, MLK.MLK 9, MLK.HOH 30, MLK.GOV 14, MLK.INV 15, MLK.EXT 4, CAP.BRD 20, CAP.MLK 30, LAB.BRD 15, LAB.MLK 25, IDT.BRD 5, IDT.MLK 4, TRF.BRD 1, TRF.MLK 2, HOH.CAP 50, HOH.LAB 40, GOV.IDT 9, GOV.TRF 3, GOV.HOH 23, INV.HOH 17, INV.GOV 2, INV.EXT 12, EXT.BRD 13, EXT.MLK 11, TRF.CAP 0, TRF.LAB 0, TRF.IDT 0, TRF.TRF 0, TRF.HOH 0, TRF.GOV 0, TRF.INV 0, TRF.EXT 0, IDT.CAP 0, IDT.LAB 0, IDT.IDT 0, IDT.TRF 0, IDT.HOH 0, IDT.GOV 0, IDT.INV 0, IDT.EXT 0, EXT.CAP 0, EXT.LAB 0, EXT.IDT 0, EXT.TRF 0, EXT.HOH 0, EXT.GOV 0, EXT.INV 0, EXT.EXT 0, INV.BRD 0, INV.MLK 0, INV.CAP 0, INV.LAB 0, INV.IDT 0, INV.TRF 0, INV.INV 0, LAB.CAP 0, LAB.LAB 0, LAB.IDT 0, LAB.TRF 0, LAB.HOH 0, LAB.GOV 0, LAB.INV 0, LAB.EXT 0, GOV.BRD 0, GOV.MLK 0, GOV.CAP 0, GOV.LAB 0, GOV.GOV 0, GOV.INV 0, GOV.EXT 0, BRD.CAP 0, BRD.LAB 0, BRD.IDT 0, BRD.TRF 0, HOH.BRD 0, HOH.MLK 0, HOH.IDT 0, HOH.TRF 0, HOH.HOH 0, HOH.GOV 0, HOH.INV 0, HOH.EXT 0, CAP.CAP 0, CAP.LAB 0, CAP.IDT 0, CAP.TRF 0, CAP.HOH 0, CAP.GOV 0, CAP.INV 0, CAP.EXT 0, MLK.CAP 0, MLK.LAB 0, MLK.IDT 0, MLK.TRF 0/
    sigma(i) /BRD 3, MLK 3/
    psi(i) /BRD 2, MLK 2/
    eta(i)
    phi(i)
...
stat_d(i).. sum(j, ((-1) * ((1 - eta(i)) / eta(i) * pd(j))) * nu_eqRT(j)) + ((-1) * (gamma(i) * (deltam(i) * m(i) ** eta(i) + deltad(i) * d(i) ** eta(i)) ** (1 / eta(i)) * 1 / eta(i) / (deltam(i) * m(i) ** eta(i) + deltad(i) * d(i) ** eta(i)) * deltad(i) * d(i) ** eta(i) * eta(i) / d(i))) * nu_eqpqs(i) + nu_eqD(i) + ((-1) * (theta(i) * (xie(i) * e(i) ** phi(i) + xid(i) * d(i) ** phi(i)) ** (1 / phi(i)) * 1 / phi(i) / (xie(i) * e(i) ** phi(i) + xid(i) * d(i) ** phi(i)) * xid(i) * d(i) ** phi(i) * phi(i) / d(i))) * nu_eqpzd(i) + nu_eqDs(i) - piL_d(i) =E= 0;
...
stat_pd(i).. sum(j, ((-1) * (d(j) * (1 - eta(i)) / eta(i))) * nu_eqRT(j)) + ((-1) * (q(i) * (gamma(i) ** eta(i) * deltad(i) * pq(i) / (1 / eta(i) * pd(i))) ** (1 / (1 - eta(i))) * 1 / (1 - eta(i)) / (gamma(i) ** eta(i) * deltad(i) * pq(i) / (1 / eta(i) * pd(i))) * ((-1) * (gamma(i) ** eta(i) * deltad(i) * pq(i) * 1 / eta(i))) / (1 / eta(i) * pd(i)) ** 2)) * nu_eqD(i) + ((-1) * (z(i) * (theta(i) ** phi(i) * xid(i) * (1 + tauz(i)) * pz(i) / pd(i)) ** (1 / (1 - phi(i))) * 1 / (1 - phi(i)) / (theta(i) ** phi(i) * xid(i) * (1 + tauz(i)) * pz(i) / pd(i)) * ((-1) * (theta(i) ** phi(i) * xid(i) * (1 + tauz(i)) * pz(i))) / pd(i) ** 2)) * nu_eqDs(i) - piL_pd(i) =E= 0;
...
stat_pq(i).. sum(j, ((-1) * ax(i,j)) * nu_eqpzs(j)) + ((-1) * (((-1) * (mu(i) * (td + sum(j, tz(j)) + sum(j, tm(j)) - sg))) / pq(i) ** 2)) * nu_eqXg(i) + ((-1) * (((-1) * (lambda(i) * (sp + sg + epsilon * Sf))) / pq(i) ** 2)) * nu_eqXv(i) + ((-1) * (((-1) * (alpha(i) * (sum(h, pf(h) * FF(h)) - sp - td + sum(j, rt(j))))) / pq(i) ** 2)) * nu_eqXp(i) + ((-1) * (q(i) * (gamma(i) ** eta(i) * deltam(i) * pq(i) / ((1 + taum(i)) * pm(i))) ** (1 / (1 - eta(i))) * 1 / (1 - eta(i)) / (gamma(i) ** eta(i) * deltam(i) * pq(i) / ((1 + taum(i)) * pm(i))) * (1 + taum(i)) * pm(i) * gamma(i) ** eta(i) * deltam(i) / ((1 + taum(i)) * pm(i)) ** 2)) * nu_eqM(i) + ((-1) * (q(i) * (gamma(i) ** eta(i) * deltad(i) * pq(i) / (1 / eta(i) * pd(i))) ** (1 / (1 - eta(i))) * 1 / (1 - eta(i)) / (gamma(i) ** eta(i) * deltad(i) * pq(i) / (1 / eta(i) * pd(i))) * 1 / eta(i) * pd(i) * gamma(i) ** eta(i) * deltad(i) / (1 / eta(i) * pd(i)) ** 2)) * nu_eqD(i) - piL_pq(i) =E= 0;
...
stat_rt(j).. ((-1) * taud) * nu_eqTd + ((-1) * ssp) * nu_eqSp + sum(i, ((-1) * (pq(i) * alpha(i) / pq(i) ** 2)) * nu_eqXp(i)) + nu_eqRT(j) =E= 0;
