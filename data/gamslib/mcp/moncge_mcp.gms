...
stat_pd(i).. sum(j, ((-1) * (d(j) * (1 - eta(i)) / eta(i))) * nu_eqRT(j)) + ((-1) * (q(i) * (gamma(i) ** eta(i) * deltad(i) * pq(i) / (1 / eta(i) * pd(i))) ** (1 / (1 - eta(i))) * 1 / (1 - eta(i)) / (gamma(i) ** eta(i) * deltad(i) * pq(i) / (1 / eta(i) * pd(i))) * ((-1) * (gamma(i) ** eta(i) * deltad(i) * pq(i) * 1 / eta(i))) / sqr(1 / eta(i) * pd(i)))) * nu_eqD(i) + ((-1) * (z(i) * (theta(i) ** phi(i) * xid(i) * (1 + tauz(i)) * pz(i) / pd(i)) ** (1 / (1 - phi(i))) * 1 / (1 - phi(i)) / (theta(i) ** phi(i) * xid(i) * (1 + tauz(i)) * pz(i) / pd(i)) * ((-1) * (theta(i) ** phi(i) * xid(i) * (1 + tauz(i)) * pz(i))) / sqr(pd(i)))) * nu_eqDs(i) - piL_pd(i) =E= 0;
...
stat_rt(j).. ((-1) * taud) * nu_eqTd + ((-1) * ssp) * nu_eqSp + sum(i, ((-1) * (pq(i) * alpha(i) / sqr(pq(i)))) * nu_eqXp(i)) + nu_eqRT(j) =E= 0;
