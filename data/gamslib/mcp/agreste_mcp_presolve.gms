...
piL_xcrop.l(p,s)$(abs(xcrop.l(p,s) - xcrop.lo(p,s)) < 1e-6) = abs(xcrop.m(p,s));
piL_xliver.l(r)$(abs(xliver.l(r) - xliver.lo(r)) < 1e-6) = abs(xliver.m(r));
piL_lswitch.l(s)$(abs(lswitch.l(s) - lswitch.lo(s)) < 1e-6) = abs(lswitch.m(s));
piL_cons.l(dr)$(abs(cons.l(dr) - cons.lo(dr)) < 1e-6) = abs(cons.m(dr));
piL_sales.l(c)$(abs(sales.l(c) - sales.lo(c)) < 1e-6) = abs(sales.m(c));
piL_flab.l(tm)$(abs(flab.l(tm) - flab.lo(tm)) < 1e-6) = abs(flab.m(tm));
piL_tlab.l(tm)$(abs(tlab.l(tm) - tlab.lo(tm)) < 1e-6) = abs(tlab.m(tm));
piL_plab.l$(abs(plab.l - plab.lo) < 1e-6) = abs(plab.m);
piL_pdev.l(ty)$(abs(pdev.l(ty) - pdev.lo(ty)) < 1e-6) = abs(pdev.m(ty));
piL_ndev.l(ty)$(abs(ndev.l(ty) - ndev.lo(ty)) < 1e-6) = abs(ndev.m(ty));
...
stat_lswitch(s).. sum(sp, ldp(sp,s) * lam_landb(sp)) - piL_lswitch(s) =E= 0;
