...
stat_p(i).. x(i) * nu_absorption(i) + sum(j, (-1) * io(i,j) * nu_actp(j)) + sum(j, (-1) * imat(i,j) * nu_pkdef(j)) + cd(i) * nu_cdeq(i) + ((-1) * gd(i)) * nu_gruse + dst(i) * sum(j, kio(j) * nu_prodinv(j)) - piL_p(i) =E= 0;
