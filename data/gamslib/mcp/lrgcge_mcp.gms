...
Parameters
    SAM(u,v) /BRD.BRD 21.0, BRD.MLK 8.0, BRD.HOH 20.0, BRD.GOV 19.0, BRD.INV 16.0, BRD.EXT 8.0, MLK.BRD 17.0, MLK.MLK 9.0, MLK.HOH 30.0, MLK.GOV 14.0, MLK.INV 15.0, MLK.EXT 4.0, CAP.BRD 20.0, CAP.MLK 30.0, LAB.BRD 15.0, LAB.MLK 25.0, IDT.BRD 5.0, IDT.MLK 4.0, TRF.BRD 1.0, TRF.MLK 2.0, HOH.CAP 50.0, HOH.LAB 40.0, GOV.IDT 9.0, GOV.TRF 3.0, GOV.HOH 23.0, INV.HOH 17.0, INV.GOV 2.0, INV.EXT 12.0, EXT.BRD 13.0, EXT.MLK 11.0, LAB.CAP 0.0, LAB.LAB 0.0, LAB.IDT 0.0, LAB.TRF 0.0, LAB.HOH 0.0, LAB.GOV 0.0, LAB.INV 0.0, LAB.EXT 0.0, HOH.BRD 0.0, HOH.MLK 0.0, HOH.IDT 0.0, HOH.TRF 0.0, HOH.HOH 0.0, HOH.GOV 0.0, HOH.INV 0.0, HOH.EXT 0.0, MLK.CAP 0.0, MLK.LAB 0.0, MLK.IDT 0.0, MLK.TRF 0.0, TRF.CAP 0.0, TRF.LAB 0.0, TRF.IDT 0.0, TRF.TRF 0.0, TRF.HOH 0.0, TRF.GOV 0.0, TRF.INV 0.0, TRF.EXT 0.0, INV.BRD 0.0, INV.MLK 0.0, INV.CAP 0.0, INV.LAB 0.0, INV.IDT 0.0, INV.TRF 0.0, INV.INV 0.0, GOV.BRD 0.0, GOV.MLK 0.0, GOV.CAP 0.0, GOV.LAB 0.0, GOV.GOV 0.0, GOV.INV 0.0, GOV.EXT 0.0, BRD.CAP 0.0, BRD.LAB 0.0, BRD.IDT 0.0, BRD.TRF 0.0, CAP.CAP 0.0, CAP.LAB 0.0, CAP.IDT 0.0, CAP.TRF 0.0, CAP.HOH 0.0, CAP.GOV 0.0, CAP.INV 0.0, CAP.EXT 0.0, EXT.CAP 0.0, EXT.LAB 0.0, EXT.IDT 0.0, EXT.TRF 0.0, EXT.HOH 0.0, EXT.GOV 0.0, EXT.INV 0.0, EXT.EXT 0.0, IDT.CAP 0.0, IDT.LAB 0.0, IDT.IDT 0.0, IDT.TRF 0.0, IDT.HOH 0.0, IDT.GOV 0.0, IDT.INV 0.0, IDT.EXT 0.0/
    Y0(j)
    F0(h,j)
    X0(i,j)
    Z0(j)
...
stat_pf(h).. sum(j, ((-1) * (((-1) * (beta(h,j) * py(j) * y(j))) / pf(h) ** 2)) * nu_eqF(h,j)) + ((-1) * (taud * FF(h))) * nu_eqTd + ((-1) * (ssp * FF(h))) * nu_eqSp + sum(i, ((-1) * (pq(i) * alpha(i) * FF(h) / pq(i) ** 2)) * nu_eqXp(i)) + nu_pf_fx_LAB - piL_pf(h) =E= 0;
...
stat_pq(i).. sum(j, ((-1) * ax(i,j)) * nu_eqpzs(j)) + ((-1) * (((-1) * (mu(i) * (td + sum(j, tz(j)) + sum(j, tm(j)) - sg))) / pq(i) ** 2)) * nu_eqXg(i) + ((-1) * (((-1) * (lambda(i) * (sp + sg + epsilon * Sf))) / pq(i) ** 2)) * nu_eqXv(i) + ((-1) * (((-1) * (alpha(i) * (sum(h, pf(h) * FF(h)) - sp - td))) / pq(i) ** 2)) * nu_eqXp(i) + ((-1) * (q(i) * (gamma(i) ** eta(i) * deltam(i) * pq(i) / ((1 + taum(i)) * pm(i))) ** (1 / (1 - eta(i))) * 1 / (1 - eta(i)) / (gamma(i) ** eta(i) * deltam(i) * pq(i) / ((1 + taum(i)) * pm(i))) * (1 + taum(i)) * pm(i) * gamma(i) ** eta(i) * deltam(i) / ((1 + taum(i)) * pm(i)) ** 2)) * nu_eqM(i) + ((-1) * (q(i) * (gamma(i) ** eta(i) * deltad(i) * pq(i) / pd(i)) ** (1 / (1 - eta(i))) * 1 / (1 - eta(i)) / (gamma(i) ** eta(i) * deltad(i) * pq(i) / pd(i)) * pd(i) * gamma(i) ** eta(i) * deltad(i) / pd(i) ** 2)) * nu_eqD(i) - piL_pq(i) =E= 0;
...
stat_y(j).. nu_eqpy(j) + sum(h, ((-1) * (pf(h) * beta(h,j) * py(j) / pf(h) ** 2)) * nu_eqF(h,j)) + nu_eqY(j) - piL_y(j) =E= 0;
