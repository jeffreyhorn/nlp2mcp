...
r.fx('i25') = 0;
theta.fx('i25') = 3.141592653589793;
...
stat_r(i)$(ord(i) <= card(i) - 1).. ((-1) * (0.5 * sin(theta(i+1) - theta(i)) * r(i+1) * 1$(j(i)))) + nu_r_fx_i25$(sameas(i, 'i25')) + sum(j, ((2 * r(i) - cos(theta(j) - theta(i)) * r(j) * 2) * lam_distance(i,j))$(ord(j) > ord(i))) - piL_r(i) + piU_r(i) =E= 0;
stat_theta(i)$(ord(i) <= card(i) - 1).. ((-1) * (0.5 * r(i+1) * r(i) * cos(theta(i+1) - theta(i)) * (-1) * 1$(j(i)))) + nu_theta_fx_i25$(sameas(i, 'i25')) + lam_ordered(i) + ((-1) * lam_ordered(i-1))$(ord(i) > 1) + sum(j, (((-1) * (2 * r(i) * r(j) * ((-1) * (sin(theta(j) - theta(i)))) * (-1))) * lam_distance(i,j))$(ord(j) > ord(i))) - piL_theta(i) + piU_theta(i) =E= 0;
