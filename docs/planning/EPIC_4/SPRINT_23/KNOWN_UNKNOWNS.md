...
| ID | Discovery | Impact | Tracker / Follow-up |
|---|---|---|---|
...
| KU-28 | Dynamic `.up` bounds not resolved by KKT generator | paperco generates `comp_up_purchase(p).. 0 - purchase(p) =G= 0` (upper=0 for positive vars) | Unfiled |
| KU-29 | Concrete element offsets in stationarity (e.g. `i1+1`, `s1-1`) | polygon, cclinpts generate invalid GAMS indexing | Unfiled |
| KU-30 | SetMembershipTest evaluation for dynamic subsets | Sets with empty `members` at compile time now raise error to fall back to GAMS evaluation; addressed in PR #1198 | Resolved |
| KU-31 | LP fast path doesn't fully eliminate translation timeouts | lop, mexls still timeout at 300s despite LP fast path; PR #1185 landed partial fix | #1169, #1185 |
| KU-32 | Emitter generates duplicate `.fx` statements | springchain has overlapping fixation blocks from multiple stationarity conditions | Unfiled |
