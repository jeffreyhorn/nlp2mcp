...
- **8 of 10 fail before PATH runs** due to execution errors (6 models: division by zero, `+-infinity * 0 is undefined`, domain violations) or MCP pairing errors (2 models: empty equations).
...
- **1 model (elec) reaches PATH but fails convergence** — Solver Status 4 (Terminated by Solver), Model Status 6 (Intermediate Infeasible), 503 iterations.
