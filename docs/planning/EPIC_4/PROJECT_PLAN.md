...
- **Phase 0 acceptance gate (PR20):** the re-emit is **O(active=398), not O(369K)** — `sarf_mcp.gms` completes in single-digit seconds; `stat_task` matches the banked 7-term derivation with **symbolic** multiplier indices (`grep -E 'nu_[[:alnum:]_]+\("|lam_[[:alnum:]_]+\("' sarf_mcp.gms` empty); byte-stable golden; determinism ×3. Banked: `SPRINT_37/DAY6_FAWLEY_LANDING.md` §1–4 (Day 7 sections), `ISSUE_1385`, `SARF_REARCH_REFRESH.md`.
...
### Priority 5: camcge Epic-5 Scoping + turkey Testbed (~10–14h)  [Sprint-37 carryforward]
...
- **Deliverable:** the camcge Epic-5 handoff document (refutations + per-model-numéraire fallback, #1330 Epic-5-scoped) + turkey's +1 realized on a licensed testbed OR re-documented as license-gated + ≥1 residual model re-triaged.
...
- camcge Epic-5 handoff (refutations + per-model numéraire, #1330 scoped) + turkey testbed +1 or license-gated re-documentation (Priority 5)
...
## Acceptance Criteria
- **Solve:** ≥ 108 (maintain the S37 close; **no carryforward is scoped to raise it** — turkey's +1 [P5] is licensed-testbed-gated and rocket's is consultation-gated [P3])
- **Match:** ≥ 94 (maintain the S37 close as-measured)
- **Genuine floor:** **maintain ≥ 76** — explicitly **not** a growth target this sprint; ganges is 0-bucket, and any advance would come from an unbudgeted P8 lever. The figure must be recomputed **from the provenance file** (P6c), not a mechanical DB count
