...
**Note:** The six retrospective carryforwards (Priorities 1–6) are the core; Priorities 7–8 pull the adjacent general-emit backlog + the infrastructure the Sprint 29 retrospective recommends (property-test catalog extension for the new head-offset/offset-alias cross-term shapes; a Sprint-30 re-baseline of the stale Rolling-KPIs Match targets; a solution-forcing harness scaffold that feeds the renumbered Sprint 36 PATH-consultation work). The Sprint 29 diagnostic tooling (KKT-residual harness, presolve-divergence detector, golden-staleness gate, `--resolve-changed` checkpoint re-solve) is reused throughout rather than rebuilt. The PATH-author-consultation / quality / release work that previously occupied Sprints 34–36 moves to **Sprints 36–38** — see the renumbered sections below (the new **Sprint 31** below is the Sprint-30-carryforward sprint).
...
- **Deliverable:** rocket → match via a documented forcing strategy (+1 Solve / +1 Match) OR a documented "needs a solver-side option beyond nlp2mcp" finding that seeds the Sprint-36 PATH consultation; the forcing scaffold is reusable for the cold-convex Case-c residue.
...
- rocket #1462 forcing → the finalized PATH-consultation input for Sprint 36 (Priority 6)
...
- **Deliverable:** the Case-c finding formalized (harness auto-classification + `ISSUE_1236` closure as documented-non-convex); the family handed to the Sprint 36 forcing/PATH consultation.
...
- rocket #1462 finalized PATH-consultation input for Sprint 36 (Priority 4)
...
- Sprint 32 finalized rocket's PATH-consultation input (Case-c boundary re-confirmed; `--force` scaffold emits; every emittable lever ruled out) and closed the hhfair + CGE cluster as documented Case-c (auto-classified `case_c_objdef`). **Submit** the packaged rocket consultation input to the Sprint-36 PATH-author consultation, and **exercise the `--force` scaffold** (homotopy / multistart / optfile) on rocket + the hhfair/CGE Case-c family — the presolve-recovered non-convex models whose only remaining avenue is forcing/reformulation.
...
- **Deliverable:** the rocket consultation input submitted (feeds Sprint 36); any `--force` lever that crosses recovers a model (+Solve) OR the forcing survey is banked for the PATH consultation; #1462 handed to Sprint 36, #1236 forcing-explored.
...
- rocket #1462 consultation input submitted (feeds Sprint 36) + hhfair/CGE #1236 Case-c forcing explored (Priority 5)
...
- **Process:** the genuine-floor ramp anchor re-baselines to **74** at S33 open (footnote ⁸); the head-offset / sarf / fawley property fixtures guard P1/P2/P3; the rocket consultation input is submitted to the Sprint-36 PATH consultation
- **Quality:** all quality gates pass; all fixes have regression tests; emit-touching PRs pass the golden-staleness check (PR26) + the presolve-divergence detector + the `--resolve-changed` checkpoint re-solve
...
- **rocket #1462 → the Sprint-36 consultation:** the FINALIZED PATH-consultation input is submission-ready; **submit** it (the concrete question + the ruled-out-lever survey + the reproducible case + the `--force` scaffold) to the Sprint-36 PATH-author consultation. Banked: `ROCKET_CASEC_FORCING_PLAN.md` + `SPRINT_32/ROCKET_PATH_CONSULTATION_INPUT.md`.
...
- **Process:** the genuine-floor ramp anchor re-baselines to **75** at S34 open; the head-offset / sarf / fawley / bound-transfer property fixtures guard the landed tracks; the rocket consultation input is submitted to the Sprint-36 PATH consultation
...
- **Final Parse Rate:** ≥ 100% of pipeline scope (confirmed from Sprint 37)
- **Final Translate Rate:** ≥ 97% of parsed models (confirmed from Sprint 37)
