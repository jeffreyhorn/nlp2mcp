...
**The finalized PATH-consultation question (feeds Sprint 32):** *"rocket's MCP is MS 5 with `EXIT — other error` at an ill-conditioned initial Jacobian from division-by-variable terms (`1/m(h)` in the velocity update, `1/ht(h)²` in gravity). `proximal_perturbation`/`merit_function`/`crash_method` move INFES 477 → 382 but do not converge from the NLP-optimum warm-start; a `1/m` auxiliary-variable reformulation `[result — in-sprint]`. Which PATH option set / regularization schedule / model reformulation forces convergence for this division-by-variable optimal-control MCP?"* (The `[result]` slot is filled by the in-sprint reformulation probe.)
