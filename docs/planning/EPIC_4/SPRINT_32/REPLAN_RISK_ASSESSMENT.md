...
**The decisive mitigation (Sprint-31 §4 + the Sprint-30 §3 lesson):** each track now carries a **precisely-pinned root cause**, so Sprint 32 implements against specifications, not open questions — mine's residual is localized to the presolve bound-multiplier transfer (`emit_gams.py:1548–1577`, `piL_x/piU_x = ±x.m` vs the residual `N`, Task 3); sarf's blow-up is the 369,024-instance 4-D `stat_task`, sparsifiable to **398 active** (Task 4); camcge is a **split** track — step 1 (`nu_mps_fx.l = -mps.m`, `mps.m = −209.861`) is a general emit fix that clears the CASE_B residual regardless, step 2 the Epic-5 dual-consistent Walras (Task 5). So REPLAN risk is **bounded**, and every REPLAN hands cleanly to a Sprint-33 filing rather than a dead end.
