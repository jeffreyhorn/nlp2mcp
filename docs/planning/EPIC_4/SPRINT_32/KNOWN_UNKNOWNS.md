...
**Decision (Day-0 / prep — SUPERSEDED by the in-sprint outcome below):** PROCEED to the in-sprint parametric `stat_task` emit; the O(active) translate-budget gate (Task 8) is the decisive test.

**🔴 IN-SPRINT OUTCOME (Day 6, 2026-07-14): tractability gate NOT met → REPLAN → Sprint 33.** Profiling pinned the blow-up in `compute_constraint_jacobian` (>120s), not the stationarity build. The 2-D constraint gate (`_is_blowup_2d_condition_equation`, extends the 1-D short-circuit; fires sarf-only `tbal/equipb1/equipb2`, no false-fire) is **necessary but insufficient** — `compute_constraint_jacobian` still times out >90s because the 369K `task` columns enumerate via `acost3` (a scalar `sum((g,t,m,n)$taskposs(g,t), oc·task)` the gate doesn't touch) + the variable path. The full fix — eliminate the 369K `task`-column materialization everywhere + emit a symbolic parametric `stat_task$taskposs` + `task.fx` — is a from-scratch symbolic-emit subsystem (the design's re-scoping exit). **REPLAN; no `src/`** (the insufficient gate was reverted); sarf stays `translate_timeout`, Translate maintains 135. See `SARF_TRANSLATE_REPLAN.md`.

**Risk-layer — Verified by:** Task 9 (REPLAN Risk Assessment). **Date:** 2026-07-14. **Findings:** Prior of REPLAN **Medium-High** — sarf is a failed-architecture rebuild (the Sprint-26 `nu_slack("srn")` set-name-literal + combinatorial-blow-up failure), and the 369K enumeration is the worst-case instance of that axis. Single-model validation: the Day-0 timing probe — `sarf_mcp.gms` translates in seconds (srpchase's 1-D analogue 6.56s), O(active=398) not O(369K); resolves the dominant risk before any downstream emit work. Sprint-33 exit: a timeout re-trigger → a documented parametric-emit re-scoping (builder-pipeline constraint); +Translate deferred, sarf stays `translate_timeout`. Reallocation: sarf's ~8–16h → P6 + P7 (the +Translate stretch is the lowest-leverage KPI). **Decision (Task-9 pre-sprint prior — SUPERSEDED by the Day-6 in-sprint outcome above):** PROCEED (the pre-outcome prior); the V1 O(active) timing probe caps the prior early. Translate +1 is conditional on this track. In the event the timing probe re-triggered the timeout → the track REPLAN'd (see the in-sprint outcome above).
...
**🔴 IN-SPRINT OUTCOME (Day 5, 2026-07-14): gate NOT met → REPLAN → Epic 5.** The `/tmp` prototype — step 1 (landed on main, PR #1553) + the consumption-weighted numéraire (`sum(i$cles(i), cles(i)·(p(i)−pd0(i)))=0` + `nu_numeraire` + the `cles(i)·nu_numeraire` cross-term in `stat_p`, every market row kept) — reaches **omega 191.7346 (correct allocation)** but stays **MS-4** (residual Walras singularity on the accounting identities `gdp`/`depreq`/`hhsaveq`/`gruse`). The re-scoped `stat_mps`-first hypothesis is **refuted** (fixing `stat_mps` first does not change the MS-4 outcome). The dual-consistent redefinition is genuinely deeper Epic-5 research → **REPLAN**, camcge stays `model_infeasible`, +1 Solve deferred to Epic 5; step 1 landed regardless. See `CAMCGE_WALRAS_REPLAN.md`.
