...
**Date:** 2026-07-13
**Findings:** The bound-active `stat_x` **can** be reconciled by construction. The harness reproduces the Day-3 fingerprint exactly (CASE_B, `stat_x(3,1,1)` rel 2.37 / raw −3.2e4, dual-transfer CONSISTENT, dual scale 1.35e4). The residual localizes entirely to `stat_x` rows with the duals CONSISTENT, so `lam_pr`/`pr.m` are correct and the mismatch is the **warm-start bound-multiplier transfer** at `emit_gams.py:1548–1577`: it sets `piL_x/piU_x = ±x.m` (the LP reduced cost), but at mine's degenerate LP vertex `x.m ≠ N` (the non-bound part of `stat_x` = `−obj_grad + Σ_k[lam_pr(k,l,i-li,j-lj)$c − lam_pr(k,l-1,i,j)$c]`), so `stat_x = N − (±x.m) ≠ 0`. The fix: derive `piL_x = max(N,0)`, `piU_x = max(−N,0)` — which closes `stat_x = N − piL_x + piU_x = 0` exactly and respects the complementarity pairing.
**Evidence:** `docs/planning/EPIC_4/SPRINT_32/MINE_BOUND_MULTIPLIER_DESIGN.md` §1–§3 (harness output + the emitted `stat_x` + the emit-site trace `emit_gams.py:1548–1577`).
