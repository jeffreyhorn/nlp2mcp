...
**Correction 1 — the rule must be calibrated.** The population is **21** Phase-0 docs, discovered with a **prefix** heading match — that qualifier is itself the first finding: `$`-anchoring the regex discovers only 20, silently dropping `ISSUE_1330` (its heading carries a parenthetical suffix), i.e. calling a doc "missing Phase 0" when it has one. Measured over all 21 against pre-remediation content: *exactly 4 `###`* → **20/21** (fails `ISSUE_1224`, which has 6 — the canonical 4 plus two narrative refresh sections); *the 4 canonical names exactly and in order* → **17/21** (also fails `ISSUE_1330`, whose *subsection* names carry the same parenthetical suffixes); *the 4 canonical names present with extras allowed* → **19/21**. Adopted the last: **prefix matching at both the heading and subsection level, extras allowed**.
