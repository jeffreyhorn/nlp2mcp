...
- **Property-test fixtures (Task 9):** add `shape7_offset_alias_cyclic.gms` (himmel16 `i++1`) + `shape8_offset_alias_successor.gms` (polygon `ord(j)=ord(i)+1`) to `tests/integration/emit/test_ad_crossterm_shapes.py` to guard the fix.
