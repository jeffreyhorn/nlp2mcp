...
| **L13** | #1283 (parser non-determinism) | chenery, indus, indus89, clearlak | model_infeasible / path_syntax_error | Non-deterministic table parsing for multi-row-label rows. Per Prep Task 3, fix is Option D (post-parse disambiguation in `_resolve_ambiguities()`). ~3–4h + 1h Option E (PYTHONHASHSEED determinism regression test). _Doc currently says OPEN; verify on Day 11 baseline._ | **1–2** (→ solve) if the doc's OPEN status still reflects the current baseline; otherwise 0 |
...
| #1311 | completed doc (`docs/issues/completed/ISSUE_1311_*.md`) — **Status:** RESOLVED | CLOSED — Day 8 abel-reassess | confirmed |
| #1312 | completed doc (`docs/issues/completed/ISSUE_1312_*.md`) — **Status:** RESOLVED | CLOSED — Day 8 abel non-convex marker | confirmed |
