...
| 35 | 35–36 | S34 carryforward — Quality, performance & PATH-feedback integration; P4 ganges/gangesx [live bucket gate], P3 fawley constraint-index-diagonal, P5 camcge [Epic 5]/rocket [PATH], P6 ganges-cohort recovery | Solve **108** / Match **93** / **floor 75** (**FLAT — 0 bucket move through Day 10**; turkey +1 pending a v54 testbed re-solve) · model_infeasible 7 · path_syntax_error 7 · _provisional, closes Day 13_ | **P6 turkey `$161`** domain-less 2-D-set COMPILE-recovery (`_infer_domainless_tuple_arity` in `_format_set_declaration`, gated on `not set_def.domain`; compile-clean — solve+match is a >1000-row testbed step) · GAMS demo **v53→v54.2.1** CI/local transition (license-expiry cascade, 22→0 local, workflows bumped) | **P4 ganges/gangesx** [**≥5-blocker cascade BANKED** — `$141`/`$145`/`$149` fixed → `$66` cold → `rPower` presolve; `$149` `_diff_prod` fix verified+banked → S36 recovery bundle] · **P3 fawley** [control 473→**1.14e-13** VERIFIED; general src/ predicate leaks onto markov #1110 → **DEFER** to dedicated effort] · camcge [Walras → Epic 5] · **rocket+mine+fawley** [S36 consultation/forcing bundle] · turkpow/clearlak/dinam/indus [heavily multi-root DEFER] · v53→v54 baseline review [Day-13/S36] |
