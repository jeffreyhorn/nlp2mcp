...
| **4.4** | ✅ **VERIFIED** | The min-convention gate is localized to `src/emit/emit_gams.py:1590` (`piL`, `and var.m > 0`) + `:1603` (`piU`, `and var.m < 0`) in `_emit_nlp_presolve` — the **sole** fix surface (re-confirmed live; the **inequality**-multiplier warm-start transfer already uses `abs()` at `src/emit/emit_gams.py:1574`). |
