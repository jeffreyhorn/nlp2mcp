...
**Day-6 executed (2026-07-21) — REPLAN (scope/risk, design sound).** Re-confirmed the three sites live (S1 `acost3` body-diff `compute_constraint_jacobian`; S2 `enumerate_variable_instances` `index_mapping.py:327` — called per-variable from `build_index_mapping`; S3 the per-column stationarity) + the absent variable-blowup gate (only the **equation** gate `_is_blowup_dynamic_subset_equation` exists). **Finding:** `enumerate_variable_instances` is **foundational** (feeds `build_index_mapping`'s `col_to_var` for every variable/model), so making `task` symbolic cascades a **coordinated re-architecture** of the column-index → Jacobian → gradient → stationarity flow (a new symbolic-column concept + a new parametric cross-term path that does not exist today), atomic (no safe partial — the gated constraints emit zero per-instance entries), touching the AD core exercised by all 142 models — for the **lowest-leverage KPI** (+1 Translate, no Solve/Match). Not safely landable in-session; a rushed foundational change risks the 135 byte-stable models. **REPLAN (scope/risk — the 7-term design is sound, not refuted)**, re-affirming S32/S33 Day-6; **no `src/`**; Translate holds 135; ~9–20 h → P6/P7. `DAY6_PROGRESS_NOTES.md`.
...
**Day-5 executed (2026-07-20) — DEFER (risk/reward, re-affirming S33); the gate-leak REPLAN exit fired on the shared-machinery blast radius.** The gap is re-confirmed live (mbal has `$(sameas(cfq__,cf))`, qsb/pbal don't — `data/gamslib/mcp/fawley_mcp.gms:238`). The fix surface was **directly examined**: the qsb/pbal cross-terms flow through the **#1104/#1111 offset-group / fresh-alias machinery** in `_add_indexed_jacobian_terms` (shared with mbal/cesam2/camcge/ps2), and the constraint-index diagonal is a **new pattern** beyond every existing guard (#1049 fires only when `len(var_domain) > len(mult_domain)` — the *opposite* of qsb, whose 3-D constraint exceeds the 2-D `bq`). A leak-free implementation needs a dedicated effort + a 2-D-cohort regression harness; the blast radius is high (a correctness-regression risk on currently-passing 2-D models) for **zero in-sprint bucket** (H-b). No `src/` shipped; the correction is fully characterized for a dedicated effort. `DAY5_PROGRESS_NOTES.md` §3.
...
**Decision (superseded by the Day-11 execution above):** ~~a single fix recovers the ganges/gangesx pair~~ — **refuted**. The `$141` fix surface identified at prep (the NaN-sanitization pass, `src/emit/original_symbols.py` — skip params whose value depends on a variable level) is **correct and verified** (removes ganges's 15 `$141`), but it is **only one of three independent roots** and recovers **no** model on its own (the `$145` universal-set cleanup and the deep `$149` CES/LES stationarity bug remain). It is **banked, not shipped** (0 bucket, un-regenerable slow-emit goldens). See the Day-11 block above and `DAY11_PROGRESS_NOTES.md`.
