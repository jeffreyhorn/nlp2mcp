...
- **The Day-0 code anchor is the S33-close SHA** (derive it with `git log --grep='SPRINT 33 CLOSED' --format=%H | tail -1`). Unlike Sprint 33, the DB is **no longer byte-unchanged since `4cbf8bff`** — the S33 P6 sample fix changed `sample_mcp.gms` + the DB, so `4cbf8bff` is historical; use the S33-close SHA for the `--resolve-changed` baseline.
...
1. Derive the Day-0 code anchor (the S33-close SHA) and verify no `src/`/`scripts/` drift since — reuse the committed DB (no fresh retest) if clean:
   ```bash
   S33=$(git log --grep='SPRINT 33 CLOSED' --format=%H | tail -1)
