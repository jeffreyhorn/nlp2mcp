...
2. **H_dual structural prototype.** Hand-edit a **scratch copy** of the emit — `/tmp/mine_mcp_prototype.gms` (+ `/tmp/mine_mcp_presolve_prototype.gms`), **not** the committed `data/gamslib/mcp/mine_mcp.gms` — so the precedence complementarity is **head-anchored** (`comp_pr` at `(k,l+1,i,j)` paired to `x(l+1,i,j)`; the `stat_x` cross-term built against the head-anchored `lam_pr`). Run the presolve prototype **from the repo root** (the emit `$include "data/gamslib/raw/mine.gms"` is repo-relative — `SPRINT_33/DAY1_PROGRESS_NOTES.md` §1). *Gate (reframed):* the **cold** MCP reaches **MODEL STATUS 1 at profit 17500** (`modelstat=1` asserted) — **not** `N → 0` at the warm point (§4.1: the warm residual is keying-invariant and is the wrong diagnostic). Confirm the 22 boundary rows close in the *cold* solution and interior rows are unperturbed.
