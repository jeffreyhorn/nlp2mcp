...
- **`src/ir/`** — carry a **head-label-indexed** multiplier domain for the precedence complementarity. The natural carrier is the S31 `EquationDef.head_domain_offsets` (`src/ir/parser.py`), currently **unused by the emit** — it already describes, per domain position, the head offset (`l+1`) that distinguishes the head label from the body label. H_dual is the first consumer of that IR in the KKT/emit layer.
...
| **1.4** | ✅ **VERIFIED** | `EquationDef.head_domain_offsets` exists (`src/ir/parser.py`) but is **unused by `src/kkt/stationarity.py`** (0 hits, re-confirmed live). It is the natural carrier for the head-label-indexed multiplier H_dual needs; H_dual is its **first emit-layer consumer** — new IR/emit plumbing is required (not a free existing capability). |
