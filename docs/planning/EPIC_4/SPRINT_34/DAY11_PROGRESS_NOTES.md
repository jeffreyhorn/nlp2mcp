...
**Fix (proposed/banked — NOT in the tree; the `src/` was reverted, see §3):** in `emit_post_assignment_na_cleanup`, skip params whose assignment references a **variable attribute** (`.l`) — via a proposed helper (`_param_assignment_references_varref_attr`, mirroring the existing `_param_assignment_has_division`, using the existing `_expr_contains_varref_attribute`). Such a param is `.l`-calibration (presolve-gated, emitted separately), **not** the #1322 data-division-NA pathology (Table-derived zero-divisor vectors), so the cleanup correctly skips it. (The cleanup is also emitted *before* the calibration in code order, so for `.l`-params it was mis-ordered — never doing useful NA-cleanup — making the skip safe.) **This helper does not exist in `src/` — it was written, verified, then reverted (banked); do not go looking for the symbol in-tree.**
...
The fix (the proposed `_param_assignment_references_varref_attr` helper + the skip in `emit_post_assignment_na_cleanup`) was written and **verified**, then **reverted (banked) — it is NOT in the tree.** Three converging reasons:
...
**Banked artifact (the de-risked hand-off — a proposal, not in-tree):** the exact fix — a proposed `_param_assignment_references_varref_attr(param_def)` helper (to mirror the existing `_param_assignment_has_division`, using the existing `_expr_contains_varref_attribute`) + a skip in `emit_post_assignment_na_cleanup` — is documented verbatim above (§2), was verified to remove ganges's 15 `$141`, then reverted; it is ready to re-apply in the dedicated ganges-recovery effort (which can afford the slow golden regen and tackles `$145`/`$149` together). The only symbols that exist in `src/` today are `_param_assignment_has_division`, `_expr_contains_varref_attribute`, and `emit_post_assignment_na_cleanup` (all pre-existing); the new helper is proposed.
