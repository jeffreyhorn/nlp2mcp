...
Fixed a `path_syntax_error` emit bug: the variable-init pass passed through an expression `.l` init referencing a variable **pruned** from the MCP. `sample.gms` has two models (`sample` uses `n`, `sampler` uses `nr`); the last solve translates `sampler`, pruning `n`, but the emit carried `c.l = sum(h, data*n.l(h))` → `$140 Unknown symbol`. **Fix** (`src/emit/emit_gams.py`): skip an expression `.l` init whose `.l`-refs aren't a subset of `_declared_mcp_vars` (from `kkt.stationarity`; narrower than `kkt.referenced_variables`). sample → MS-1 @ 726.679 = NLP optimum (match). Blast radius = sample only; det ×3; `make test` 5,034 passed. `DAY11_P6_COHORT.md`.
