...
| **2.3** | ✅ VERIFIED | The banked 7-term `stat_task` is symbolic — every multiplier over the stat domain (`nu_tbal(g,t)`, `lam_equipb1(m,t)`, `nu_acost3`, …), **no** quoted-set-name indices; the compile-clean anti-pattern scan (§4 gate item 2 — an explicit `grep -E` over `sarf_mcp.gms`) is the structural guard against the `243fe578` anti-pattern. |
