...
| Step | Gate | Measurable by |
|---|---|---|
| V1 | Re-confirm the Day-11 control (<code>max&#124;stat_bq&#124;</code> 473 → 18, 96%) and **localize the residual 18.47 by column** — the H-a/H-b discriminator | Day-0/early |
| V2 | The full generalization (every second-index `cfq` gets the `$(sameas(cfq__,cf))` restriction, covering qsb/pbal) drives **<code>max&#124;stat_bq&#124; → 0</code>** (not 96%) at the warm point (`modelstat` asserted). **H-a:** presolve → **MS-1 @ 2899.25** (+1 Solve). **H-b:** <code>max&#124;stat_bq&#124; → 0</code> yet MS-5 → emit ships genuine, +Solve → P5. **No regression:** `--resolve-changed --since-commit ee51ed9e` GO (polygon/ps2 the 1-D core untouched; **no mbal-term change** — Unknown 3.3 structurally favorable) | **Day-5 checkpoint** |
