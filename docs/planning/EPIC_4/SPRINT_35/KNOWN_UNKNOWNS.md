...
**Task-11 contribution (2026-07-24):** the measured golden-regen budget **makes P4 shippable in-sprint and un-constrains its scheduling** — ~8.2 min scoped fits a normal ≤ 12 h day (no overnight slot), so P4 can be **front-loaded Days 1–5** (surfacing the `$149` REPLAN gate by the Day-5 checkpoint) without reserving a nightly window, and the S34-P4 "no bucket → no `src/`" exception is *satisfiable* if a bucket move materializes (`REPLAN_RISK_ASSESSMENT.md` §P4 + front-load ordering). Primary: Task 3.
