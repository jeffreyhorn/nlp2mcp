...
**Disposition:** hand mine to the **Sprint-36 PATH-author consultation** as the canonical primal-degenerate-LP question (alongside rocket's Case-c), and reallocate the **18–24 h P1 budget → P4** (Task 5's ganges/gangesx recovery, the designated best shot) first, then P6/P7. Surfacing the REPLAN in **prep** (vs S34's Day-1) lets P4 plan against the full freed budget from Day 0. mine stays `model_infeasible`; no `src/`; P1 contributes 0 in-sprint Solve / 0 genuine floor (handed to Tasks 9/11/12). The IR (`head_domain_offsets`) is sufficient but moot — nothing survives to carry it. Standing BANs (sign flip; `x.up=inf`) restated, not reconsidered.
