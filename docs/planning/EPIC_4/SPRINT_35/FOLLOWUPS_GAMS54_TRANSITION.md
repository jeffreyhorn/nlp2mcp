...
**RESOLVED (Day 11) — neither regression nor stale assertion: a never-emitted `CASE_B` emit bug, now a banked +1-floor lever.** Day-11 archaeology + the KKT-residual control settled both branches of the question: (1) the test assertion is **correct** — the intended `#1110` diagonal split (`+nu_constr(s,i)` direct + off-diagonal sum with `nu_constr(s__kkt1,j)`) matches a hand-derivation of markov's stationarity; (2) that split was **never once emitted** (test red from birth `c5958aee`, hidden by `slow`; `db385a43`'s #1110 "fix" only corrected the `pi` index, leaving the Kronecker `1` inside the sum). The KKT-residual control confirms the current emit is a genuine cold-emit bug: **`CASE_B`, `max|stat_z|` rel 13.3** (the Kronecker `1` is summed `card(s)=8`× over the enumeration dummy). markov is a *methodology* match (presolve-rescued); a correct emit ⇒ `CASE_A` ⇒ cold `model_optimal` ⇒ **genuine floor 75→76 (+1)**. Because the fix is a deep repair of the shared multi-pattern (#1110) machinery (high-blast-radius across the matching 2-D cohort — the fawley Day-9 leak precedent), it is **banked to a dedicated Sprint-36 effort**, not landed. Full diagnosis + implementer-ready hand-off: [DAY11_MARKOV_DIAGONAL_LEVER.md](DAY11_MARKOV_DIAGONAL_LEVER.md). The `slow`/`xfail` disposition of the test is decided **with** the fix (the assertion is correct as-is).
