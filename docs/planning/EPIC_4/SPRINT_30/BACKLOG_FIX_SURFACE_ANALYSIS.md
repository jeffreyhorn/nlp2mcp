...
**Shared root cause (Unknown 7.1 context):** both himmel16 (#1146) and polygon (#1143) are harness-**Case b** on a single `stat_*` row, both **already match warm** (cold-robustness / genuine-floor, NOT +Match), both route through `src/ad/derivative_rules.py` (`_diff_varref`, `_partial_collapse_sum`) + `src/kkt/stationarity.py` (`_replace_indices_in_expr`). **But they are distinct shapes with distinct remaining bugs** (below), so the "one fix, two models" framing from Sprint 29 is **refined**: they share the code path, not the exact defect.
