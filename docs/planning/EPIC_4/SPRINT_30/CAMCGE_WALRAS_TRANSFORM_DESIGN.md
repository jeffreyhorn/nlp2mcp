...
> **⚠️ Sprint 30 Day-11 empirical refinement (2026-07-08) — the `drop-row` step breaks the MCP DUAL; REPLAN → Sprint 31.** A ~2h empirical run (hand-applied to the emitted MCP) confirmed the premise (NLP optimum `p=pd0=1`) and that **pinning the price ray reaches the correct allocation** (fix `p('services')=pd0` OR the §3.2 numéraire eq → **omega 191.735**), **but MODEL STATUS 4 persists** (residual Walras redundancy; PATH stuck, large `gdp` INFES; proximal_perturbation does not rescue it). Crucially, **§3.1's `drop-one-market-clearing-row` corrupts the MCP**: it is solution-preserving for the PRIMAL (as proven below) but each market-clearing multiplier is a **needed dual in the stationarity** (`nu_lmequil(lc)` = the wage in `stat_l(i,lc)`; `nu_equil(i)` = the goods price in ~6 `stat_*` rows), so dropping a row + fixing its multiplier zeros a needed price/wage → omega **299** (broken). **The paper solution-preservation (§3.1) verified the primal but omitted the MCP dual side.** The real fix is a **dual-consistent** redefinition (express the dropped market's multiplier via Walras' law so it stays available in the stationarity), not a naive row-drop — deeper Epic-5 MCP research. See `ISSUE_1330` Day-11 block. The sections below stand as the primal argument + the (valid) numéraire selection; §3.1's drop-row needs the dual refinement before it lands.

## 0. Executive summary

`CGE_DEGENERACY_SCOPING.md` proved (on paper) that camcge's MS-4-at-iteration-0 is an **inherent CGE Walras-law rank-deficiency** — the goods-market `equil(i)` and labor-market `lmequil(lc)` clearing rows are linearly dependent given household budget balance (one row is redundant), and no price numéraire is fixed (CGE equilibria are homogeneous of degree 0 in prices) → a singular KKT Jacobian PATH cannot pivot from. The fix is a **CGE-domain structural preprocessing transformation** (drop one redundant market-clearing row + fix a price numéraire), solution-preserving on paper **for the primal** — **but the Day-11 empirical run (banner above) found the naive `drop-row` step breaks the MCP *dual* (it orphans a needed price/wage multiplier); it is NOT ready to implement as-stated and needs a dual-consistent refinement before it lands.** This task resolves the three `§5` open questions into an implementation design and **grounds it in camcge's actual structure**, surfacing one refinement:
...
| **S2 — singular-Jacobian solve signature** (corroborating) | The full-MCP `kkt_residual.py` verdict + a **cold PATH solve**: **MODEL STATUS 4 Infeasible at iteration 0** with a uniform `stat_*`-row INFES (camcge's `stat_cd` uniform −0.2022) and a PATH **basis-singularity** report. | Confirms the rank-deficiency actually defeats PATH (not just a benign near-dependency). *Insufficient alone:* MS-4-at-iter-0 can also come from a genuine emit bug (`CASE_B`) — must be paired with S1 (rank) + the residual-clean check to isolate *structural* singularity from an emit defect. |
...
**Solution-preservation (paper).** CGE equilibrium conditions are **homogeneous of degree 0 in prices**: scaling all domestic prices by `λ>0` leaves every excess-demand and zero-profit condition unchanged, so equilibria form a **ray** `{λ·p* : λ>0}`; the real allocation (all quantities `x`,`cd`,`l`,…) is identical along the ray. The numéraire constraint selects the single representative whose consumption-basket index matches the base-year value `sum(i$cles(i), cles(i)*pd0(i))` — i.e. it fixes `λ` to that normalization (`λ = 1` only when the unscaled equilibrium already satisfies it, e.g. if calibration is itself an equilibrium at `p = pd0`; `pd0` is emitted as a *warm-start* `p.l(i)=pd0(i)`, not a fixed condition, so `λ` is whatever value makes the basket index match, not assumed to be 1). Crucially, **because quantities are invariant along the ray, welfare is independent of which representative the numéraire picks** — so it is a **selection, not a perturbation**, and the Cobb-Douglas welfare `omega = prod(i$cles(i), cd(i)**cles(i))` evaluates to the equilibrium optimum (the P6 Day-0 run confirms **191.7346**). Combined with §3.1, the reduced square MCP is **non-singular** and its unique solution is camcge's NLP equilibrium.
