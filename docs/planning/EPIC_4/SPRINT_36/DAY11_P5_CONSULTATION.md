...
⇒ **No forcing strategy closes the H-b divergence** — every mode stays **MS-5 Locally Infeasible** (the objectives wander but none reaches the LP optimum 2899.25; the H-b baseline is MS-5 @ 4399.557). This confirms fawley's H-b diagnosis (Task 4 §5): the `stat_trans(tr-2)` residual is the **emit-correct non-emit divergence** — the MCP is correctly emitted but PATH cannot reach the LP optimum from the warm start, and the current `--force` scaffold (μ-continuation / perturbed-restart / proximal-optfile) does not force it. **0 in-sprint bucket**; the +Solve hands to a Sprint-37 consultation (a stronger continuation / a reformulation), NOT a `stat_bq` emit fix.
