...
**Measured this task:** the committed camcge MCP golden generates **641 single (scalar) equations and 641 single variables** under GAMS 54.2.1 demo (the GAMS listing reads `BLOCKS OF EQUATIONS 85 / SINGLE EQUATIONS 641` — the 641 generated scalar equations are grouped into 85 symbolic *blocks*; the two are **not additive**, and the GAMS demo size limit is measured on the generated **single/scalar** count, not the block count). PATH runs to **SOLVER STATUS 1 (Normal Completion) / MODEL STATUS 4 (Infeasible)** — the exact cold MS-4 signature — with "*This solver runs with a demo license.*" **641 generated rows < the 1000-row demo limit.**
