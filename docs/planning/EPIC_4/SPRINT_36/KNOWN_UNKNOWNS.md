...
**Findings:** There is **no licensed >1000-row GAMS-54 testbed** in the current infrastructure. Both environments probed on `main` are GAMS **demo** (1000-row solve limit): the local install (`Versions/54/Resources/gamslice.txt` = `GAMS Demo`, 54.2.1, time-limited to Nov 26 2026) and **both** CI workflows (`pr19-emit-solve-validation.yml`, `presolve-divergence.yml`) run a step literally named "Install GAMS demo" (54.2.1 `linux_x64_64_sfx.exe`). **But the impact is narrow:** the Solve 108 / Match 93 baseline was itself built under a demo license (DB `gams_version` = `51.3.0`) → every *currently-solving* model is ≤ the 1000-row limit → the v54 re-baseline of the solving set is fully demo-runnable (local + CI). Only turkey (3,866-row MCP, `path_syntax_error`, *not* in the 108) is genuinely license-gated.
