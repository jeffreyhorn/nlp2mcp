...
**Findings:** **(Q3, corrected)** the camcge MCP golden generates **SINGLE EQUATIONS 641 / SINGLE VARIABLES 641** under GAMS 54.2.1 demo and PATH runs to SOLVER STATUS 1 / MODEL STATUS 4 (Infeasible) — 641 rows **< the 1000-row demo limit**. This **supersedes the `DAY8` claim** that "camcge's MCP exceeds the demo 1000-row limit" — the generated MCP is demo-solvable, so the Walras `/tmp` MS-1 control is a **local demo step, NOT a licensed-testbed step** (contrast turkey/6.1). **(Q1)** the S1∧S2∧S3 detector still fires only camcge (cold MS-4 @ omega 191.7346), see 5.4. **(Q2)** the price-pin variant reaches the correct primal but stays MS-4 (row-redundancy nullspace, not fixed by a numéraire). **(Q4)** the per-model-numéraire declaration (`../../EPIC_5/CGE_DEGENERACY_SCOPING.md`) is the fallback if MS-1 is unreachable.
