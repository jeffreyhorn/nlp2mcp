...
> **Note on the historical `/tmp/sprint25-golden/` reference:** earlier drafts of this section pointed at `/tmp/sprint25-golden/<model>_mcp.gms`, which was the Sprint 25 Day 0 golden directory generated by `SPRINT_25/PLAN.md` Day 0 setup. That `/tmp/` directory is ephemeral and not guaranteed to exist for Sprint 26 prep work. Use `data/gamslib/mcp/<model>_mcp.gms` (the committed artifacts from current main) as the baseline instead — these are always available and reflect the latest merged state.
