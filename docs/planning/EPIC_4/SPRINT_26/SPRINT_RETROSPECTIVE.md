...
| Solve | 103 | ≥ 111 (= ≥82% of 135 translated per PROJECT_PLAN.md S27; **+6 firm Solve gains + 2 conditional**: firm = #1381 camcge/cesam2 [+2 — solve from path_syntax_error, conditional only on the Phase B fix landing] + #1398 qdemo7 recovery [+1 — qdemo7 was matching at Day 0 before #1398 regressed it, so fixing #1398 returns it to match→solve] + #1357 otpop [+1] + #1356 fawley [+1] + #1388 camshape [+1 — solve from model_infeasible Locally Infeasible]; **conditional** = #1385 Option 1 short-circuit [+1 — unblocks iswnm/mexls/nebrazil/sarf translates; conditional on at least one of the 4 subsequently solving cleanly in MCP — no current evidence either way] + #1224 mine ParamRef IndexOffset [+1 — unblocks mine translate from `translate_internal_error`; **conditional on mine subsequently compiling AND solving cleanly in PATH** — currently translate.failure, solve untested, no evidence either way; documents an unsupported translate-stage `IndexOffset(ParamRef)`, not a solve-time fix]. **Reaching +8 requires BOTH conditional sources to convert to actual Solves**; if only one converts, target is +7 (104 Solve, ~77%, below ≥82%). **#1378 launch PATH-numerics recovery is NOT a Solve gain — launch already solves at Day 13 (`mcp_solve.status: success`, `outcome_category: model_optimal_presolve`); #1378 moves launch from mismatch to match (counted in Match target below)**. AD-architecture redesigns #1390 kand / #1393 / #1335 are also mismatch-to-match improvements (NOT Solve gains — those models already solve). #1398 widened-scope egypt/ferts/shale remain blocked by `path_solve_license` even with Phase A compile fix, so they don't add Solve gains.) |
| Match | 59 | ≥ 66 (= ≥46% of 142 pipeline scope per PROJECT_PLAN.md S27; +7 via #1381 [+2] + #1398 qdemo7 [+1] + #1357 [+1] + #1356 [+1] + #1378 launch mismatch→match [+1] + #1390 kand mismatch→match [+1]; #1388 camshape is +1 Solve but mismatch-rate-vs-NLP not yet measurable [it's currently Locally Infeasible, so no NLP comparison baseline]) |
