...
**Day 11 Status:** COMPLETE (2026-02-18) — Subcategory F grammar fixes: NONNEGATIVE_K (wall), eqn_head_mixed_list (robustlp, solveopt), smax/smin aggregation rules (tricp), DOLLAR NUMBER condition (solveopt), offset_expr extended (imsl), scalar_item description (imsl), $onEchoV/$offEcho preprocessor stripping (tricp), slash-in-quoted-string fix (imsl), recursion limit 10000→50000 (imsl); qdemo7 already parsing; Checkpoint 2: GO (66.5% parse rate, 27 lexer_invalid_char, 6 internal_error, all P1-P3 done); 3,553 tests pass
**Day 12 Status:** COMPLETE (2026-02-18) — IndexOffset AD Integration (Part 1): _substitute_index() helper + _apply_index_substitution() extended for IndexOffset bases in VarRef/ParamRef and DollarConditional; 23 unit tests (22 pass, 1 xfail for sum-collapse-with-IndexOffset-wrt deferred to Day 13); 3,576 tests pass
**Day 13 Status:** COMPLETE (2026-02-18) — IndexOffset Validation: offset_paren/offset_func parser fix (expr_fn callback pattern); 8 blocked models evaluated (launch/robert/pak: TRANSLATE OK; mine/ampl/tabora/otpop/sparta: 5 issues filed #780-#784); 4 new tests (TestOffsetParenIndexing); lint fix (Callable import); 3,579 tests pass
**Day 14 Status:** COMPLETE (2026-02-18) — Issues #780-#784 fixed (PR #786): ref_indexed offset_func, set ordinal attrs, leading-zero range, eqn attr assignment, sum multi-index dollar scope; Issues #774+#766 fixed (PR #787): shale singleton equation quoted domain, robert stat_x c(p,tt)→c(p,t) subset guard; close #671 (already resolved — orani fully resolved by prior ISSUE_670 fix); 5 remaining issues documented as deferred (#753/#757/#763/#764/#765); parse rate 107/160 (66.9%) ✅; 3,579 tests pass; sprint COMPLETE
