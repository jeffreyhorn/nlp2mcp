...
**The organising fact.** `dedupe_repeated_variable_domains` (#1062, `src/cli.py:476`) runs unconditionally before differentiation and iterates `model_ir.variables` **only**. So the **variable**-domain sub-shape is globally neutralised and the **set** (16 models) and **parameter** (21 models) sub-shapes are not. Most `NEEDS A TEST` verdicts are "safe today, but by something upstream and incidental".
...
**⚠ The live finding.** The emit-level property finds **9 violations in 6 CURRENT goldens**. `dinam`, `egypt`, `turkpow` carry repeats the **source never declared** — manufactured by our emit; `shale`, `gussrisk`, `nonsharp` carry declaration-derived ones. (`nonsharp` is 3-arity and was invisible to the original binary matcher — found only when PR review forced it to be generalised.) **11 of the 34 models match, but only `gussrisk` is in the suspect set, and its instance is latent** (an NA-guard narrowed to the diagonal, on data that is never NA). Of the other **five**, four are `mcp_solve: failure` and `nonsharp` is convexity-`excluded` with no MCP solve — so no reported KPI is affected today.
...
- [x] The corpus used as the false-positive test set — 180 parsed models; `korcge` is the decisive control, structurally a near-twin of camcge and well-posed
