...
**Correction 1 is APPLIED** — PREP_PLAN's Task 6 verification block now greps `"= enumerate_variable_instances("` in `gradient.py` and `complementarity.py`, and carries the inverted-rationale note so it is not reintroduced. The `= ` anchor matters: an unanchored grep also picks up three `>>>` docstring examples in `index_mapping.py` and a prose mention at `gradient.py:48`.
