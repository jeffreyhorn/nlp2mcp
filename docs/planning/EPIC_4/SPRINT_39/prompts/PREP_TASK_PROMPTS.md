...
**Unknowns to verify:** **6.1** (has a reply arrived, and would it be actionable — check #1462 and #1443), **6.2** (if a licence arrives, does the 11-model batch actually solve, and is a per-model sanity check warranted before spending capacity), **6.3** (does `agreste` need a new owning issue).
