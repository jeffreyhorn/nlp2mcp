...
| `make test` | ✅ **5177 passed, 10 skipped** (see below) |
...
- `tests/unit` alone → **3725 passed, 0 skipped**: every attribution test ran (9 at the time of that check; the module has since grown to **98 collected cases** across the review rounds).
...
**Rounds 3–7 each passed first time on a quiet machine** — 5112, 5125, 5138, 5146, 5154, 5166, 5170, then **5177 passed, 10 skipped, exit 0**, each exactly the previous total plus that round's new tests, with `-rs` naming the same 10 pre-existing skips. Applying the rule above rather than re-deriving it saved the re-run.
