...
- **A gate's traced fix surface was wrong about the layer on three consecutive days** — tricp (D11), elec (D12, twice), dyncge (D12). The first three under-scoped (naming the emitter for defects decided upstream in AD/KKT); dyncge **over**-scoped, demanding "new logic" for a diagonal-triviality test that had existed since #942 and was merely applied to the wrong population.
