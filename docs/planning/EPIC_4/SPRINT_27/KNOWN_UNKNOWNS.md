...
- **Research Q1 (combined effort estimate):** Per Task 8's per-issue effort breakdown:
  ...
  - #1388 camshape ~4.5h Case (a) emit bug + NLP-warm-start solves (incl. 1h Day 0/1 diagnostic step), ~5.5h Case (b) emit bug + warm-start guidance, ~1.25h Case (c) fundamental property + Sprint 28 carryforward filing. Binding case selected by Day 0/1 NLP-warm-start runtime test (REVISED 2026-05-29 per PR #1409 review — boundary-guard mechanism downgraded to UNPROVEN; actual cause requires Day 0/1 diagnosis).
