...
3. Dry-run the script with both `--since-date "2026-04-22"` and `--since-commit <sprint-26-day-0-sha>` against Sprint 26 history to verify both modes surface the same `data/gamslib/mcp/*.gms` artifacts regenerated during Sprint 26. Expected artifacts (16 total, since launch is NOT one of the 15 #1398-affected models — it's a separate Phase A target whose emit was regenerated on Day 1): `launch_mcp.gms` + `launch_mcp_presolve.gms` (Phase A target, Day 1 PR #1379) + all 15 #1398-affected models' `*_mcp.gms` (Day 13 regenerated: qdemo7 / egypt / ferts / shale / sambal / qsambal / harker / tfordy / dinam / ganges / gangesx / fawley / srpchase / sroute / turkpow). #1400 is a `scripts/gamslib/*` path-relativization change and will NOT appear in the script's output by design (the script only scans `data/gamslib/mcp/*.gms`).
