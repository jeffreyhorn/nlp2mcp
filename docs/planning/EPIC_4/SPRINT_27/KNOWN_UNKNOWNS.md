...
- **Q2 (projected widened runtime, Option A):** ~37s steady state. Breakdown: 22s setup + 2s Tier 0/1 (11 models @ ~0.04s) + ~10s Pattern C (19 models incl. launch — launch ~0.2s MODEL STATUS 5, 9 compile-fast @ ~0.02s + 2 license-fail @ ~0.5–1s + 3 solve-pass @ ~0.5–2s + buffer) + 3s overhead. Worst case (all 19 Pattern C hit reslim timeout): ~19min — within 20-min hard ceiling but tight (mitigated by observed compile-fast/fail-fast behavior). **(Corrected Day 0: launch is pattern-c, not Tier 0/1 — MODEL STATUS 5 Locally Infeasible.)**
