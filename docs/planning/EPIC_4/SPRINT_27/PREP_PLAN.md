...
Computed combined-budget fit across all 4 scenarios (most-likely / mixed / worst / worst-worst) using REVISED #1388 effort estimates (now including a Day 0/1 diagnostic step since the boundary-guard hypothesis was disproved). Most-likely path: both Sprint 27 fix (~10.5–11.5h within 6–12h budget — tight at upper end; +1 Match + +1 Solve gains). Mixed path: #1387 fix + #1388 carryforward Case c (~7.25h; +1 Match only). Worst path: #1387 AD-architecture escalation + #1388 fix (~5.5–6.5h, lower end of budget; +1 Solve only). Worst-worst: both deferred (~2.25h filing). All paths within budget.
...
**Combined Priority 7 effort (most-likely path):** ~10.5–11.5h within 6–12h budget (tight at upper end). **Sprint 27 gains (most-likely path):** +1 Match (cclinpts) + +1 Solve (camshape) = 2 headline-metric improvements.
...
- [x] Combined Sprint 27 effort fits within Priority 7's 6–12h budget — §5 4-scenario table; most-likely path ~10.5–11.5h within budget (tight at upper end); all 4 scenarios within budget
