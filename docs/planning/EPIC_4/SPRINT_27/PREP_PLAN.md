...
Pulled the 42 top-level Copilot review comments on Sprint 26 PR #1396 (PR19 CI extension — `.github/workflows/pr19-emit-solve-validation.yml` + `scripts/ci/parse_pr19_targets.py` + `scripts/ci/run_pr19_solves.py`) via `gh api repos/jeffreyhorn/nlp2mcp/pulls/1396/comments`. Categorized the 42 comments across the 7 recurring categories from the Sprint 26 retrospective. Per-category comment counts (single primary-category assignment per comment, no double-counting; see `docs/planning/EPIC_4/SPRINT_27/PR23_CHECKLIST_DESIGN.md` §2 for the authoritative tables and §2.9 for the totals): input validation, pagination, fork tolerance, schema validation, error handling, marker uniqueness, logging visibility — with 4 cross-cutting "Other" comments folded into existing categories per `docs/planning/EPIC_4/SPRINT_27/PR23_CHECKLIST_DESIGN.md` §2.8 (placeholder-SHA256 blocking-default → logging + error handling; `--soft-fail` vs `--tier soft-fail` flag inconsistency → input validation; 12-minute job timeout < worst-case runtime → error handling). Comment counts and category totals are maintained in one place — `docs/planning/EPIC_4/SPRINT_27/PR23_CHECKLIST_DESIGN.md` §2 — to avoid drift between docs; this PREP_PLAN.md §Changes paragraph deliberately does not duplicate the numbers.

Authored new top-level §"CI Workflow PR Checklist (PR23, Sprint 27 Prep Task 10)" section in CONTRIBUTING.md (inserted before the existing §"Emit-PR `.gms` Diff Workflow (PR22, ...)" section), comprising: (a) brief rationale paragraph naming the Sprint 26 PR #1396 incident; (b) explicit scope (`.github/workflows/*.yml` / `.github/workflows/*.yaml` — which is also where reusable workflows live — plus `scripts/ci/*` and `.github/actions/*` — which is where composite actions live); (c) how-to-use note for PR authors and reviewers ("`N/A — <reason>` instead of leaving unchecked"); (d) the 7-category checklist totalling 32 specific actionable items.

Each category's items are a mix of (i) literal restatements of the PR #1396 issue and (ii) defense-in-depth extensions generalizing the same root cause to related failure modes. Per-category item counts: Input validation 5, Pagination 3, Fork tolerance 4, Schema validation 5, Error handling 5, Marker uniqueness 4, Logging visibility 6. Six categories sit within the prescribed 3–5 range; **Logging visibility at 6 is the documented exception** because it absorbed the most PR #1396 comments (the largest per-category cluster) — see `docs/planning/EPIC_4/SPRINT_27/PR23_CHECKLIST_DESIGN.md` §2.9 for the matching note and acceptance-criteria amendment below.
...
| Scope clauses | `.github/workflows/*.yml` / `.github/workflows/*.yaml` (also where reusable workflows live) + `scripts/ci/*` + `.github/actions/*` (also where composite actions live) |
| Total checklist items | **32** (acceptance criterion: ≥ 25) |
| Category coverage | all 7 categories present; per-category item counts 5, 3, 4, 5, 5, 4, 6 — six categories within the prescribed 3–5 range, **Logging visibility at 6 is the documented exception** because it absorbed the largest PR #1396 comment cluster (see `docs/planning/EPIC_4/SPRINT_27/PR23_CHECKLIST_DESIGN.md` §2.9) |
| Source PR | Sprint 26 PR #1396 — 42 top-level review comments across 11 rounds, all categorized in `docs/planning/EPIC_4/SPRINT_27/PR23_CHECKLIST_DESIGN.md` §2 |
| Item-to-comment traceability | `docs/planning/EPIC_4/SPRINT_27/PR23_CHECKLIST_DESIGN.md` §3 maps each of the 32 checklist items back to the originating comment ID(s) or marks the item as a defense-in-depth extension |
| Sample PR self-review | `docs/planning/EPIC_4/SPRINT_27/PR23_CHECKLIST_DESIGN.md` §4 applies the checklist to a hypothetical Sprint 27 mid-sprint audit-workflow PR (27/32 checked, 5 N/A with reason) |
...
- [x] Each of the 7 categories has 3-5 specific actionable items — **as delivered: six categories within the 3–5 range and Logging visibility at 6 as the documented exception** (the category absorbed the largest PR #1396 cluster, per `docs/planning/EPIC_4/SPRINT_27/PR23_CHECKLIST_DESIGN.md` §2.9; both PREP_PLAN.md §Changes/§Result and the design doc record the exception explicitly)
- [x] Total checklist contains ≥ 25 items (delivered: 32)
- [x] Scope clearly defines applicability — full scope as delivered in CONTRIBUTING.md: `.github/workflows/*.yml` / `.github/workflows/*.yaml` (also where reusable workflows live, called via `workflow_call`) + `scripts/ci/*` + `.github/actions/*` (also where composite actions live)
- [x] `docs/planning/EPIC_4/SPRINT_27/PR23_CHECKLIST_DESIGN.md` contains the Sprint 26 PR #1396 categorization that motivated each item
