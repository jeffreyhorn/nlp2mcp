...
- Advanced Unknowns 9.1/9.2/9.3 (harness design) + 1.3/2.2/5.2 (harness-as-instrument) to 🟡 **PARTIALLY VERIFIED (design scope)** in `KNOWN_UNKNOWNS.md` — the design answers the *how*, but the implementation-time measurements (runtime, residual parity, kand dual-transfer correctness) and Day-0 probes (GAMS clip/wrap) remain pending the in-sprint build; appended a Task 4 instrument entry to 5.1.
...
- Case-calibration *targets* are pinned to Sprint 27 ground truth — camshape `stat_r('i1')`≈396 (Case b) vs cclinpts max|r|=5e-8 (Case c), which `tol=1e-6` would cleanly separate — to be confirmed empirically when the harness is built and run in-sprint (this is design, not a measured result).
...
- [x] Unknowns 9.1, 9.2, 9.3, 1.3, 2.2, 5.1, 5.2 updated in KNOWN_UNKNOWNS.md to 🟡 PARTIALLY VERIFIED (design scope) — empirical/Day-0 verification pending the in-sprint build
...
- **Detector** (to be built in-sprint) is *designed to* extract the embedded NLP objective via a uniquely-named-scalar probe (`Scalar embedded_nlp_obj_PROBE; embedded_nlp_obj_PROBE = <objvar>.l; Display embedded_nlp_obj_PROBE;`, parsed by the unique name) inserted after the `$offMulti` that closes the source `$include` (anchored on the include, since presolve files have more than one `$offMulti`; pre-MCP-Solve) and the standalone via a direct solve, and to flag relative-objective gaps > `tol` (1e-4) or worse embedded model status. **By design it would flag both past wins** — #1378 launch (2604.01 vs 2257.80) and #1424 camshape (infeasible vs MS 2, locally optimal); the pre-fix replay is the in-sprint acceptance run.
...
- **#1400 second leak — LOCATED** in the warning-capture path: `batch_translate.py:~279–286` stores subprocess `stderr` (which carries Python's default `<abspath>:<lineno>: UserWarning` format) into the `message` field; same shape on the solve path (`run_full_test.py:550`). **DB audit: current DB clean (0 leaks)** — transient defect. Candidate surface: relativize the captured text, or a repo-relative `warnings.formatwarning` in `src/cli.py`. ~1–2h, LOW coupling.
