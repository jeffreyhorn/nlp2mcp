...
2. **Local anchor pre-pass** in `_build_indexed_gradient_term` (`src/kkt/stationarity.py`) — rewrite same-set elements to `IndexOffset(base=var_indices, offset=ord(e)-ord(anchor))` so cross-terms re-symbolize to `j±k` (validated Day 8); gated to the gradient path (does NOT touch `_replace_indices_in_expr` — keeps #1452/#1335/#1393 byte-stable).
