...
**Status:** **RESOLVED (Sprint 28 Day 11, 2026-06-19).** Both shapes now fixed: the `.fx`-restore shape (Sprint 27 Day 13, otpop/dinam) and the `.l` denominator/override shape (this day, robot). Fix (`src/emit/emit_gams.py`): when merging the `.fx → .l` per-instance overrides into a variable's init group, skip an override line that exactly duplicates a denominator-guard bulk-init line already emitted for that variable (same element, same value) — e.g. robot's `rho.fx(firstlast)=4.5` → `rho.l('h0')=4.5`/`rho.l('h50')=4.5` when the divisor-guard init already set those elements to 4.5. An override with a DIFFERENT value (or for an uncovered element) is kept. Solve-equivalent (removing a redundant same-value re-assignment is a no-op at runtime); blast radius byte-only. 1 integration test (`test_1374_robot_l_init_dedup.py`). *(Prior: PARTIALLY FIXED — Sprint 27 Day 13 fixed only the `.fx`-restore shape.)*
