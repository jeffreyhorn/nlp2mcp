...
# Regression guard for the 13 presolve models that use the Layer-4 unfix:
for m in catmix himmel16 polygon bearing launch mathopt3 robustlp; do
  .venv/bin/python -m src.cli data/gamslib/raw/$m.gms --nlp-presolve -o /tmp/${m}_p.gms --quiet
done   # byte/solve-stability must hold
```
...
rocket presolve MCP → MODEL STATUS 1 with `compare_objective_match` to the NLP `final_velocity` reference, **and** zero byte/solve regression across the 13 Layer-4-unfix presolve models. If unreachable after a complete `_fx_` warm-start + bound-complementarity fix, REPLAN the non-convex convergence to Sprint 30.

## Provenance

- #1449 Layer-4 unfix (Sprint 28 Day, PR for otpop presolve) — repaired rocket's broken presolve golden (abort → MS 5, a forward step); introduced the `h0` bound-complementarity question.
