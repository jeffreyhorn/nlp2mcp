...
**GitHub:** #1462
**Status:** **REPLAN → Sprint 32 (Sprint 31 Day 11, 2026-07-12) — the LAST emittable lever (the division-by-variable reformulation) is exhausted; the finalized PATH-consultation input is the deliverable.** The Day-11 probe reformulated rocket to remove ALL division-by-variable from the initial Jacobian (`gf` → `g·ht² = g_0·h_0²`; a free acceleration `a(h)` with `(a+g)·m = T−D` replacing `(T−D−m·g)/m` in `v_eqn`). The reformulated NLP solves to the same optimum (1.0128), but its **MCP is MS-5 Locally Infeasible cold, MS-5 warm-started from the NLP optimum, AND MS-5 across every mu-continuation step** (nh=10; the nh=50 reformulated MCP exceeds the demo-license nonlinear-row limit). **So rocket's non-convergence is intrinsic to the discretized optimal-control MCP structure, NOT the division-by-variable Jacobian conditioning** — combined with the exhausted PATH-option survey (INFES 477 → 382, Sprint-30 Task 4) and continuation/multistart (all MS-5, Sprint-30 Day 3), **NO emittable lever converges rocket.** → **REPLAN: finalize the PATH-consultation input** (`BACKLOG_FIX_SURFACE_ANALYSIS.md` §3 — the reformulation is now a *ruled-out* candidate, sharpening the question toward the intrinsic structure). rocket stays `model_infeasible`; +1 Solve deferred. The `--force` scaffold (Sprint 30) + this finalized question are the de-risked Sprint-32 hand-off. Sprint targets already met (Match 95, genuine floor 74). _(was: Sprint 29 Day 2 `_fx_` warm-start LANDED; +1 Solve REPLAN-DEFERRED → Sprint 30, then the Sprint-31 PATH consultation.)_
...
> **🔄 Sprint-31 refresh (Prep Task 6, 2026-07-09): PROCEED-conditional — residual clean at the NLP point (Case-c) BEFORE forcing; the deliverable may be the PATH-consultation input.** The `--force` scaffold landed Sprint 30; no emittable-GAMS lever converged rocket (INFES 477 → 382 best, never crosses). **PROCEED gate (PR27):** re-confirm the emit residual is **clean at the NLP point (Case-c)** *before* any forcing attempt (keeps rocket a forcing problem, not a latent emit bug); then exhaust the remaining emittable levers (the `1/ht²`,`1/m²` division-by-variable Jacobian reformulation; scaled/relaxed continuation). **REPLAN** to the **finalized PATH-consultation input** for the renumbered Sprint 32 if no lever crosses. Fix surface / lever set pinned by Prep Task 9 + `docs/planning/EPIC_4/SPRINT_30/NONCONVEX_FORCING_SURVEY.md` §4. See `docs/planning/EPIC_4/SPRINT_31/PHASE_0_ACCEPTANCE_GATES.md` §P6.
>

> **🔴 Sprint-30 Day-3 DECISION — REPLAN rocket's +1 Solve to the Sprint-31 PATH consultation (2026-07-07).** With the P8 forcing scaffold landed (Day 2), Day 3 drove the **emittable-GAMS levers** the Task-4 survey left unexhausted — on rocket's `--nlp-presolve` MCP, warm-started from the NLP optimum: (1) **multistart** with a *real* `.l` perturbation of the primals (`v`/`ht`/`m`/`step` scaled ±5 %…±50 % across 6 restarts) → **all 6 restarts MS 5**, `done=0`, no convergence (confirming the survey a-priori: warm-starting from the optimum itself already fails, so perturbed restarts are unpromising); (2) **homotopy** as a `proximal_perturbation` continuation (Levenberg-Marquardt regularization `mu: 1e3 → 0`, warm-restart from each prior point, a fresh `path.opt` per step) → **all 7 continuation steps MS 5**, INFES stuck at ~458–467 (no monotone drive to convergence). **No emittable-GAMS lever converges rocket** — combined with Task-4's PATH-option result (INFES 477 → 382 best, no config crosses), rocket's MS-5 is **intrinsic non-convergence**. **DECISION: PROCEED-to-scaffold is done (firm); rocket's +1 Solve REPLANs to the Sprint-31 PATH consultation.** Scoped hand-off (unchanged from below): *which PATH option set / regularization schedule / reformulation converges the division-by-variable optimal-control MCP whose initial Jacobian carries `1/ht²`,`1/m²`?* Scaffold hardened this day: the `--force homotopy` strategy now emits the **model-agnostic proximal_perturbation continuation** (validated to run on rocket) rather than a placeholder relaxation hook. Freed budget → Day-4 hhfair.

> **🔄 Sprint-30 refresh (Prep Task 4 + Task 5, 2026-07-05) — forcing-lever survey done; rocket's +1 Solve is NOT firm for Sprint 30.** Sprint 30 Prep Task 4 (`docs/planning/EPIC_4/SPRINT_30/NONCONVEX_FORCING_SURVEY.md`) probed the tunable PATH forcing levers on rocket's presolve MCP (env-guarded `path.opt`): `proximal_perturbation` {1e-2,1e-1,1.0,1e2}, `crash_method pnewton` + `crash_perturb yes`, `merit_function normal`, and combined — **all stay MS 5**; best (`merit_function normal` + `proximal_perturbation 1e-2`) reduces INFES 477 → 382 but never converges. **No PATH-option configuration (via optfile) forces rocket** (intrinsic non-convergence confirmed; the Goddard rocket's `1/ht²`,`1/m²` ill-conditioned initial Jacobian). **Disposition (Unknown 2.2): PROCEED to the Sprint-30 P8 forcing scaffold** (the emitted-GAMS homotopy/continuation + multi-start driver — the structural levers), with the **PATH-option tuning handed to the Sprint-31 PATH-author consultation** (the concrete question: which option set / regularization schedule / reformulation converges this division-by-variable optimal-control MCP). **rocket's +1 Solve / +1 Match is DEFERRED to Sprint 31** — the Sprint-30 P2 deliverable is the scaffold, not a rocket solve. No cold-convex Case-c shared payoff (the 4 Case-c cohort models already warm-match; rocket is the sole non-converging model). Verify: `.venv/bin/python scripts/diagnostics/kkt_residual.py data/gamslib/raw/rocket.gms`.
