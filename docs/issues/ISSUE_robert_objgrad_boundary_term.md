...
- **Decisive cold-solve control** (self-contained, no dual-transfer artifact): patch `data/gamslib/mcp/robert_mcp.gms` `stat_s` objective gradient → `storage-c$(t(tt)) − res-value$(ord=card)` and re-solve cold; **expect MODEL STATUS 1 at profit 11025.0** (= NLP ref). Patching `stat_x` → `nu_sb(r,tt+1)` instead leaves 6741.67 (the control that refutes the banked claim).
