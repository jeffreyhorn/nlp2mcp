...
**Status:** **✅ CLOSED — DOCUMENTED GENUINE CASE-C, auto-classified (Sprint 32 Day 10, 2026-07-15).** The `kkt_residual.py` **Case-c auto-classifier landed** (`scripts/diagnostics/kkt_residual.py`, `reclassify_objdef_case_c`): a CASE_B whose max-residual `stat_<var>` is the objective-defining intermediate variable (**D1**: `<var>` in `obj =e= f(<var>)` with its own defining equation, so `nu_obj=±1`), with the cold-start MCP at a *spurious* optimum (**D3**: cold objective ≠ the presolve match — hhfair cold 72.147 vs match 87.159, both MS-1), is reclassified `case_c_objdef` (D2 dual-CONSISTENT implied). All four family members now auto-flag: **hhfair `stat_u(1)` rel 2.00, irscge `stat_xp(BRD)` 0.064, lrgcge `stat_xp(BRD)` 0.045, moncge `stat_xp(MLK)` 0.066** → `case_c_objdef`; a genuine emit residual on a non-objective-defining variable (camcge `stat_tm`) correctly stays `case_b` (D1 false-positive guard). **No emit fix; the SIGN FLIP STAYS BANNED** (refuted 4× S30–S31). They stay methodology (0 genuine floor), handed to the Sprint-33 forcing/PATH work like rocket. _(prior: DOCUMENTED GENUINE CASE-C, Sprint 31 Day 10 — the ν_objective reduction control-refuted before any `src/` change.)_ **This closes #1236.** The Sprint-31 P5 track (the ν_objective objective-gradient reduction for the objective-defining-intermediate-variable family, hhfair `stat_u` + the CGE cluster `stat_xp`) was **control-refuted before any `src/` change**: the reduction is inert (the CGE cluster stays cold `UU=25.5085` for both sign choices; hhfair's sign flip was already refuted 3× in Sprint 30, 72.147 → 22.144 *worse*). hhfair (CES + bilinear) and the CGE cluster (irscge/lrgcge/moncge — Scale-Economy/Large-Country/Monopoly) are **genuine non-convex Case-c**: the cold solve sits at a spurious local KKT point, and the match is reachable only via the presolve warm-start (they stay methodology, not genuine floor). P5 delivered **0** genuine floor; the SIGN FLIP STAYED BANNED (no `src/` shipped on a refuted hypothesis — the PR24/PR27 control-first discipline as intended). See the Day-10 OUTCOME block in §"PROCEED/REPLAN Signal". _(was: `$184` CLEARED Day 4; the `stat_u` obj-grad SIGN-FLIP fix REFUTED Day 6 — +1 Match NOT firm, deeper diagnosis deferred.)_
**Severity:** Medium — Model solves optimally but objective differs from NLP
**Date:** 2026-04-09
**Last Updated:** 2026-07-15 (Sprint 32 Day 10 — CLOSED: the `kkt_residual.py` Case-c auto-classifier landed [D1∧D3 → `case_c_objdef`]; all four members auto-flag, camcge guard holds; sign flip stays BANNED; prior: 2026-07-13 Sprint 31 Day 13 closeout documented genuine Case-c; 2026-07-12 Sprint 31 Day 10 REPLAN → Case-c; 2026-07-07 Sprint 30 Day 6 `stat_u` sign-flip refuted by control)
...
> **🔄 Sprint-31 refresh (Prep Task 6, 2026-07-09): PROCEED-conditional — the ν_objective reduction; control experiment BEFORE src; the SIGN FLIP IS BANNED.** The P5 track is the objective-defining-intermediate-variable obj-grad residue (hhfair `stat_u` / CGE `stat_xp`). **PROCEED gate (PR24/PR27 control):** the objective-gradient reduction **through the objective-defining-equation multiplier (ν_objective)** must reach the **NLP optimum on hhfair** in a `/tmp` control experiment **before** the objective-gradient `src/` change. **The sign flip is BANNED** — control-refuted **three times** in Sprint 30 (hhfair 72.147 → 22.144, *worse*). Then confirm the reduction converts the CGE cluster (irscge/lrgcge/moncge) to Case-a. **REPLAN** to a documented Case-c non-convexity finding if the reduction does not reach the NLP optimum. Fix surface pinned by Prep Task 9. See `docs/planning/EPIC_4/SPRINT_31/PHASE_0_ACCEPTANCE_GATES.md` §P5.
>
> **➡️ Day-10 OUTCOME (2026-07-12): CONTROL REFUTES the reduction → REPLAN the CGE cluster to Case-c (like hhfair).** The `/tmp` control (GAMS, before any `src/`): the CGE cluster **cold-mismatches** (irscge/lrgcge/moncge all cold `UU = 25.5085` vs the presolve match `26.0914`), and the ν_objective reduction is **INERT** — `UU` appears ONLY in `obj.. UU =e= prod(i, Xp(i)**alpha(i))` (so nu_obj = ±1 and the reduction reduces to the sign choice), and flipping `stat_xp`'s objective-gradient sign leaves the cold primal at the **identical 25.5085** for all three. So fixing `stat_xp` does NOT move the cold solution toward the match — the cold solve sits at a spurious local KKT point. These are **non-convex model classes** (irscge = *Scale Economy* / increasing returns, lrgcge = *Large-Country*, moncge = *Monopoly*), so 25.5085 is a genuine non-global local optimum; the match (26.09) is only reachable via the presolve warm-start. **This is exactly the hhfair Case-c pattern** (the Sprint-30-Day-6 sign-fix refutation) — the design's "CGE cluster is convex, one ν_objective reduction → several cold" hypothesis is REFUTED by the control. **The SIGN FLIP STAYED BANNED** (tested only as a control; no `src/` shipped). → **REPLAN: the CGE cluster is genuine Case-c** (stays methodology — matches via presolve, not cold). P5 delivers 0 genuine floor this sprint; genuine floor stays 74 (P2's gain). hhfair remains genuine Case-c. No `src/` landed (control refuted pre-src).
>
