...
> **🟡 NOT a missing-term bug — refined Day 4 (Sprint 29, 2026-06-29).** The #1143 representative-selection fix (which fixed polygon's *successor* offset cross-term) does **not** change himmel16 (`stat_area` still rel 2.0). Inspecting the emit: himmel16's `stat_x`/`stat_y` already carry the **circular** `i++1` decomposition — `(...)*nu_areadef(i-1))$(ord(i)>1)` (the linear predecessor) **plus** `(...)*nu_areadef(i+5))$(ord(i)<=card(i)-5)` (the boundary wrap) — so the cross-term structure is **present**, not dropped. The 2.0 residual is therefore a **numeric/sign defect** in the objvar-defining-gradient interaction (`stat_area.. -1 + nu_areadef(i)`, where the `-1` is the `obj2`/`totarea = sum(area)` gradient and the transferred `nu_areadef` carries the equality sign) — Day-0 candidate (b), confirmed. **Remaining #1146 work** = the cyclic-coefficient / objvar-gradient-sign reconciliation (a `_diff_varref(circular=True)` + dual-transfer-sign question), distinct from the #1143 successor fix and NOT addressed by it. Property fixture `shape7_offset_alias_cyclic.gms` guards the (structurally-present) circular decomposition; the numeric correctness is deferred (candidate for Sprint 30 or a later day, needs a GAMS residual evaluation to pin the sign).

- **PROCEED** — Day-0 Case b on `stat_area`, rel ≈ 2.0 (✅ confirmed). _[Day-4: confirmed NOT a missing term — the circular decomposition is present; the 2.0 residual is a numeric/sign defect, deferred.]_
