...
The MCP model declaration for circle.gms omits the pairing for the primal variable `r`: the stationarity equation `stat_r` is defined, but there is no `stat_r.r` entry in `Model mcp_model / ... /;`. This leaves variable `r` unpaired and equation `stat_r` unused, making the MCP formulation incomplete/incorrect.
...
The circle.gms model is an optimization problem where:
- `r` is a scalar primal variable (a radius or similar geometric quantity)
- `b` is another variable in the problem
...
This is the same issue pattern as GitHub #623 (cpack.gms), indicating a systematic bug in the model declaration emission logic where scalar (non-indexed) variable pairings may be handled differently than indexed pairings.
...
grep -A 20 "Model mcp_model" /tmp/circle_mcp.gms
...
- circle.gms cannot be solved as an MCP because variable `r` has no complementarity pairing
- Same root cause as GitHub #623 - fix should address both issues

## Testing

1. Unit test: Verify scalar variable pairings are included in model declaration
2. Integration test: circle.gms compiles and solves successfully after fix
3. Regression test: Models with indexed variables still work correctly

## Related Issues

- GitHub #623 - Same issue pattern in cpack.gms (fix should resolve both)

## References

- `data/gamslib/mcp/circle_mcp.gms` - Generated MCP file showing the bug
- `data/gamslib/raw/circle.gms` - Original GAMS model
