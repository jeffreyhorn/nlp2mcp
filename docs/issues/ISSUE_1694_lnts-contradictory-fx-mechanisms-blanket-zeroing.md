...
**⚠ A design note carried from the reverted attempt (PR #1732 review).** The implementation duplicated the three-part `_fx_`-survival predicate — declared ∧ not suppressed ∧ multiplier not simplified away — re-deriving it at the blanket by iterating `var_def.fx_map` a second time, while the `.fx` emission gate (~`:2358`) already evaluates exactly that test. **Two copies of one predicate will drift**, and the cost is paid again on every large `fx_map`. A landable fix should **factor the paired-in-MCP check into a shared helper**, or cache the surviving indices when the `.fx` gate runs, and consume that from the blanket. Reusing the *logic* was right (S38-D12); reusing it by **copying** was not.
