# polygon: Offset-Alias Gradient Mismatch — ✅ RESOLVED (Sprint 31 Day 4: coupled objective-successor + distance-second-index fix landed; warm-matches 0.780 ≈ NLP 0.7797)

**GitHub Issue:** [#1143](https://github.com/jeffreyhorn/nlp2mcp/issues/1143)
**Status:** **✅ RESOLVED — Sprint 31 Day 4 (2026-07-10): the coupled fix LANDED; polygon MATCHES (warm 0.780 ≈ NLP 0.7797, MCP MODEL STATUS 1).** Both halves landed together in `src/kkt/stationarity.py`: (1) **objective successor** — `_count_additive_terms` + interior-representative re-selection in `_build_indexed_gradient_term`'s offset path; (2) **distance second-index** — `_var_at_two_indices_complement` + `_build_complement_index_sum` (the complementary transpose sum `sum(j, ∂distance(j,i)/∂r(i)·lam_distance(j,i))$(ord(j)<ord(i))`, inverted multiplier + flipped ord). **Tight gate held:** excluding DIAGONAL rows keeps the CGE multi-pattern cohort (irscge/lrgcge/moncge/quocge/cesam) byte-identical (#1110 orthogonal). **Blast radius 6 goldens — all correct KKT completions of var-at-two-indices constraints** (polygon match; cpack/himmel16/ps2_f_s/ps2_s/ps3_s_gic second-index sums on packing `nooverlap`/`maxdist`; all compile clean + cold-solve MS-1, no regression; himmel16 stays non-convex). `shape10` fixture + test guards the distance half; `shape8` un-xfailed. No Sprint-32 REPLAN needed — the gate did not leak. _(was: Sprint 30 Day 8 REPLAN → Sprint 31; the objective half was implemented+reverted Day 8 pending the coupled distance half.)_
...
> **🔄 Sprint-31 refresh (Prep Task 6, 2026-07-09): PROCEED — the 4-term recipe re-confirmed on the current tree; land the coupled fix tightly gated.** Prep Task 4 (`docs/planning/EPIC_4/SPRINT_31/OFFSET_ALIAS_JACOBIAN_DESIGN.md`) re-confirmed the recipe (harness byte-identical to the Day-0 fingerprint: CASE_B, `stat_theta(i12)` rel 0.492, CONSISTENT) with **two PR24 fix-surface corrections**: the second-index drop is in `_add_indexed_jacobian_terms` (`src/kkt/stationarity.py:5767`), **NOT** `constraint_jacobian.py`; the objective half (`_count_additive_terms`) is confirmed absent on `main`. **PROCEED gate:** land the objective-successor half + the distance second-index half **together** (neither alone matches), tightly gated to var-at-two-indices; completion = `shape8_offset_alias_successor` enabled + polygon warm-match 0.780 + CGE byte-stable (#1110 orthogonal — single-scalar correction vs a whole position-keyed sum). **REPLAN** to a Sprint-32 #1111/#1112 AD-engine filing if the gate leaks. See `docs/planning/EPIC_4/SPRINT_31/PHASE_0_ACCEPTANCE_GATES.md` §P2.
>
