...
| Model | Set Size | Diagonal Elements | EXECERROR | Status |
|-------|----------|-------------------|-----------|--------|
| ps2_f_s | 2 (eff, inf) | lam_ic(eff,eff), lam_ic(inf,inf) | 2 | SOLVED (Optimal) |
| ps2_s | 2 (eff, inf) | lam_ic(eff,eff), lam_ic(inf,inf) | 2 | SOLVED (Optimal) |
| ps3_s_gic | 3 (0, 1, 2) | lam_ic(0,0), lam_ic(1,1), lam_ic(2,2) | 3 | SOLVED (Optimal) |
| ps3_s_scp | 3 (0, 1, 2) | lam_ic(0,0), lam_ic(1,1), lam_ic(2,2) | 3 | SOLVED (Optimal) |
...
Added section 2c in `src/emit/emit_gams.py` (`.fx` emission): for each inequality
complementarity equation with 2+ domain indices, resolve each index to its
canonical set (via `model_ir.aliases`). When two indices share the same canonical
set, emit:
...
- #943: Dollar-conditioned complementarity unmatched variables (fixed in same PR)
