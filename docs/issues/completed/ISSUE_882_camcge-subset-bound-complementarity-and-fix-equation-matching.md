...
**Status:** All 12 MCP unmatched equation errors are resolved by accumulated
prior fixes (empty equation detector #1133, stationarity condition fixes,
bound complementarity improvements).

**Current state (Sprint 24 retest):**
- No MCP pairing errors (0 unmatched equations)
- No `comp_lo_e`/`comp_lo_m`/`comp_lo_pm`/`comp_lo_pwe` equations generated
  (bound structure changed by prior KKT improvements)
- No `e_fx_publiques`/`e_fx_services` scalar fix equations generated
- Remaining: EXECERROR=4 from runtime division-by-zero in `stat_pd(services)`,
  `stat_pd(publiques)`, `stat_xxd(services)`, `stat_xxd(publiques)` — these are
  nonlinear KKT expressions undefined at the initial point for non-traded
  elements (same class as #1192 gtm runtime division-by-zero)
