...
**Status:** **REPLAN → Epic 5 (Sprint 31 Day 6, 2026-07-11) — the `/tmp` dual-consistent prototype did NOT reach MS-1, and the KKT-residual harness RE-DIAGNOSES camcge as CASE_B (emit_bug), not the clean Walras-singular case the design assumed.** The Day-6 substantive experiment (the Day-0-flagged prototype): the numéraire re-pairing (`numeraire ⊥ nu_numeraire`, all market-clearing rows kept — the design's "try first") reaches omega **191.7346 but stays MS-4**; a single-dual pin (`nu_equil.fx('services')`) also stays MS-4 → the dual redundancy is **deeper than a single Walras relation** (the design §5 REPLAN trigger). Decisively, the harness verdict is **CASE_B — emit_bug**: the stationarity does NOT balance at the NLP optimum (`stat_mps` rel **1.05** / raw −210; `stat_tm`/`stat_pwm` residues), with dual-transfer CONSISTENT (closure residual 4.8e-10). `mps` is a FIXED variable (`mps.fx=.09305`), so `stat_mps` carries the fixing multiplier `nu_mps_fx` — the residual is a **fixing-multiplier transfer/stationarity defect**, a *different bug class* from the Walras dual-singularity the dual-consistent transform targets. **So the design's core premise ("the warm-start IS a valid KKT point; the failure is only the singular Jacobian") is REFUTED — camcge has a genuine Case-B emit residual, and the Walras transform addresses the wrong defect.** → **REPLAN to a per-model-numéraire-declaration Epic-5 item** (the design §5 exit), re-scoped: the Epic-5 work must FIRST resolve the `stat_mps`/`nu_mps_fx` Case-B residual (re-diagnose via the harness), then the dual-consistent numéraire. camcge stays `model_infeasible`; the +1 Solve is deferred. Sprint-31 targets already met (Match 95 ≥ 92, genuine floor 74 ≥ 73 at Day 5); only the Solve ≥ 109 stretch (which needed mine [P1, REPLAN'd] + camcge) is missed. _(was: REPLAN → Sprint 31, Sprint 30 Day 11 — the Walras transform's `drop-row` step is primal-correct but breaks the MCP dual.)_ The Day-11 empirical run REFINED the design's premise: pinning the price ray (fix `p('services')=pd0` OR the consumption-weighted numéraire eq) reaches the **correct allocation (omega = 191.735)** — but **MS-4 persists** (the residual Walras row-redundancy; PATH cannot converge, `gdp` identity stuck, proximal_perturbation does not resolve it). Dropping one market-clearing row (the design's other half) **corrupts the solution** (omega → 299): every market-clearing multiplier is a needed price/wage in the stationarity (`nu_lmequil(lc)` is the wage in `stat_l(i,lc)`; `nu_equil(i)` the goods price in ~6 `stat_*` rows), so dropping a row orphans that dual. **The design's paper solution-preservation verified the *primal* but missed the MCP *dual* — the real fix is a Walras-consistent multiplier redefinition, not a naive row-drop.** See the Day-11 block below. _(was: REPLAN → Sprint 29 / Epic 5, Sprint 28 Day 11.)_

> **🔴 DECIDED — REPLAN to Sprint 31 (Sprint 30 Day 11, 2026-07-08).** Attempted the Epic-5 Walras transform empirically (hand-applied to the emitted `--nlp-presolve` MCP, ~2h of variants). **Findings:** (1) **premise validated** — NLP optimum has `p.l(i) = pd0(i) = 1` ∀i, so the numéraire target is correct; (2) **pinning the price ray → correct allocation** — fixing `p('services')=pd0('services')` or adding `numeraire.. sum(i$cles(i),cles(i)*p(i)) =e= sum(i$cles(i),cles(i)*pd0(i))` (+ its `cles(i)*nu_numeraire` cross-term in `stat_p`) both reach **omega 191.735** (= NLP ref) but stay **MODEL STATUS 4** (residual Walras redundancy; PATH stuck near the solution with a large `gdp` INFES; **proximal_perturbation does not rescue it** — a true structural rank deficiency, not a benign singular basis); (3) **the row-drop corrupts** — dropping `lmequil('rural')` + fixing `nu_lmequil('rural')=0` zeros the rural wage in `stat_l(i,'rural')` → omega **299** (broken), because **every market-clearing multiplier is a needed dual** (`nu_lmequil(lc)` = wage in `stat_l`; `nu_equil(i)` = goods price in `stat_cd/stat_dst/stat_gd/stat_id/stat_int/stat_x`). **Conclusion: the design's `drop-one-market-clearing-row + numéraire` is primal-solution-preserving on paper but breaks the MCP dual structure** (the paper analysis omitted the dual side). A clean MS-1 needs a **dual-consistent** treatment (redefine the dropped market's multiplier via Walras' law so it stays available in the stationarity, rather than orphaning it) — genuinely deeper Epic-5 MCP research, not a same-day hand-transform. **→ REPLAN to Sprint 31.** camcge stays `model_infeasible`; the refined findings + the working price-pin recipe (omega 191.735) + the exact dual-flaw are banked below and in `CAMCGE_WALRAS_TRANSFORM_DESIGN.md`. No `src/` change (all experiments were `/tmp`). *(Prior REPLAN → Sprint 29 / Epic 5 observation follows.)*

**Status (superseded):** **REPLAN → Sprint 29 / Epic 5 inherent-degeneracy observation (Sprint 28 Day 11 Task-6 gate, 2026-06-19).** The KKT-residual harness confirms the round-3 diagnosis: camcge's MCP is **structurally singular** (CGE Walras-law redundancy — `equil(i)` goods-market + `lmequil(lc)` labor-market clearing are linearly dependent given budget balance, with no price numéraire fixed), so PATH cannot pivot (cold MCP MODEL STATUS 4 Infeasible at iteration 0). **No localizable single-row emit bug** — the harness CASE_B max-residual `stat_mps` is a **fix-multiplier-transfer artifact**: `mps` is a fixed variable, `stat_mps` (`sum(i,y·cles(i)·nu_cdeq(i)) − y·nu_hhsaveq + nu_mps_fx`) hand-derives correct, and the synthetic `mps_fx` equation has no NLP marginal so `nu_mps_fx` is not warm-started → the residual (−210) equals exactly the (correct) gradient terms. **PROCEED criterion NOT met:** there is no clean "single redundant Walras row + numéraire fix preserving the economic solution" expressible as a general nlp2mcp emit change — CGE-aware Walras-redundancy elimination + price-numéraire selection is a domain-specific structural transformation = **Epic 5 scope**. Do NOT spend Sprint-28 `src/` budget. *(Prior: OPEN — three partial fixes 2026-04-30; round-3 diagnosis = singular-Jacobian PATH-convergence, not emit/AD.)*

## Sprint 28 Day 11 — Task-6 gate → REPLAN (2026-06-19)
...
**Date:** 2026-04-30
**Last Updated:** 2026-07-12 (Sprint 31 Day 6–7 — REPLAN → Epic 5; `/tmp` prototype stayed MS-4, harness re-diagnosed CASE_B `stat_mps`, cohort precision verified; prior: 2026-07-08 Sprint 30 Day 11, 2026-06-11 Sprint 28 Prep Task 5, 2026-04-30)
...
> **🔄 Sprint-31 refresh (Prep Task 6, 2026-07-09): PROCEED-conditional — the dual-consistent redefinition; prototype to MS-1 on `/tmp` BEFORE src (check the dual side).** Prep Task 5 (`docs/planning/EPIC_4/SPRINT_31/CAMCGE_DUAL_CONSISTENT_DESIGN.md`) designed the dual-consistent multiplier redefinition: keep every market-clearing row (no orphaned dual) + a consumption-weighted numéraire + redefine the redundant market's dual via Walras' law → MS-1. **PROCEED gate (PR24 control):** the redefinition must reach **MS-1 at omega 191.7346** in a hand-edited `/tmp` MCP **before** the `src/` change (the Day-11 price-pin reaches 191.735 but stays **MS-4**; the drop-row orphans a needed dual → omega 299). The **S1∧S2∧S3** detector must flag camcge only (**S3 cold-MCP-singular = the false-positive guard**). **REPLAN** to a per-model-numéraire-declaration Epic-5 item if `/tmp` can't reach MS-1. See `docs/planning/EPIC_4/SPRINT_31/PHASE_0_ACCEPTANCE_GATES.md` §P3.
>
> **➡️ OUTCOME (Sprint 31 Day 6, 2026-07-11): gate NOT met → REPLAN → Epic 5.** The `/tmp` prototype did not reach MS-1: the numéraire re-pairing and a single-dual pin both stayed **MS-4** at omega 191.7346. The harness re-diagnosed camcge as **CASE_B (emit_bug)** — a genuine `stat_mps` stationarity residual (rel 1.05, the fixed-`mps` fixing-multiplier `nu_mps_fx`), NOT the clean valid-KKT + singular-Jacobian the transform assumes. The dual-consistent Walras transform targets the wrong defect. No `src/` landed (gate failed pre-src). See the top status block. The **S1∧S2∧S3 detector was NOT built** (no src, since the gate failed).
>
> **➡️ Day-7 close (2026-07-12): REPLAN CONFIRMED + cohort precision VERIFIED.** The Day-6 single-dual pin (`nu_equil.fx('services')`) staying MS-4 also **rules out the explicit Walras redefinition** (which just pins `nu_equil(n*)` to its Walras value — the same as the warm-value pin) → no numéraire/Walras variant reaches MS-1, so the **per-model-numéraire alternative also does not land camcge** (the defect is the CASE_B `stat_mps` residual). Cohort precision test (cold MCP MODEL STATUS): **irscge / lrgcge / moncge / stdcge all MS-1 Optimal** (not Walras-singular → fail S3 → pass-through); **camcge MS-4** (would flag) → **only camcge** is the Walras case, confirming the design scope + that a per-model-numéraire declaration won't spuriously apply elsewhere. **P3 closed → Epic 5** (re-scoped: `stat_mps` Case-B first, then the numéraire); freed budget → P5/P7.
>

> **⚠️ SUPERSEDED by the Sprint 30 Day-11 status block at the top (2026-07-08).** The Prep-Task-5 "PROCEED to drop-`lmequil` + fix-numéraire" disposition below is **partially refuted**: the empirical Day-11 run confirmed the *premise* (structural singularity, `p=pd0`) and that pinning the price ray reaches the correct omega 191.735, **but the `drop-row` step breaks the MCP *dual*** (orphans a needed price/wage multiplier) → it does **not** reach MS-1 as-stated. Read the disposition below as historical; the current status + the needed dual-consistent refinement are in the top block.
>
> **🔄 Sprint-30 refresh (Prep Task 5, 2026-07-05): PROCEED to the Epic-5 Walras transformation (P6).** camcge's MCP is structurally singular (CGE Walras-law redundancy — `equil`/`lmequil` linearly dependent, no price numéraire), so no localizable single-row emit fix exists (Case-c structural, not an emit bug). Disposition: **PROCEED** to the Epic-5 CGE-domain preprocessing transformation — **drop the redundant market-clearing row (`lmequil`) + fix a price numéraire (`cpi=1`)** — which is solution-preserving on paper and reproduces camcge's NLP optimum **191.7346** (`docs/planning/EPIC_5/CGE_DEGENERACY_SCOPING.md`). **The Sprint-30 P6 design (Prep Task 7, forthcoming) resolves the open questions**: the degeneracy-detection heuristic (must NOT false-flag a well-posed model — Unknown 6.2), the redundant-row + numéraire selection rule, and the empirical confirmation (drop-`lmequil` + fix-`cpi=1` → MODEL STATUS 1 at 191.7346 — Unknown 6.1). **REPLAN to a per-model-numéraire-declaration Epic-5 item** if the detection heuristic proves unreliable. Distinct from the Class-B CGE `stat_pz` general-emit bug (`docs/issues/ISSUE_classB_cge_stat_pz.md`, NOT Walras — Unknown 7.3).
