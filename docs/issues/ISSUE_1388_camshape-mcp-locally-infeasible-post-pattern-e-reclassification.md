...
## Day 5 RESOLUTION (2026-06-16) — FIXED, +1 Solve (camshape MS 1, area 4.2841)

**Root cause (hand-derived + harness-confirmed):** the `stat_r(i)` **offset cross-terms** (from neighbor constraints `convexity(i±1)`, `convex_edge1(i-1)`, `convex_edge3(i+1)`) were guarded by the equation's membership condition at the **current** index instead of the **offset (neighbor)** index — `$(middle(i))` instead of `$(middle(i+1))`, `$(first(i))` instead of `$(first(i-1))`, etc. At `i1` (first), `middle(i1)=no` wrongly **suppressed** the `convexity(i2)` cross-term that balances the `convex_edge1(i1)` self-term → `stat_r(i1)` residual ≈ −396 (≈ −2·lam_convex_edge1).

**Fix (`src/kkt/stationarity.py`, the Issue #877 condition-propagation site):** for an offset cross-term, shift the equation condition's domain indices by the same `offset_key` used to build the multiplier (reusing `_reindex_condition_symbols` from #1224). The emit now guards the neighbor terms by `middle(i+1)`/`middle(i-1)`/`first(i-1)`/`last(i+1)`. camshape (warm-started) → **MODEL STATUS 1, obj 4.2841 = NLP match.**

**Blast radius (7 primary goldens changed; all correct, NO regression):** camshape (→ MS 1 match), kand + srkandw (unchanged, still MS 1 — their boundary multipliers were 0), clearlak + dinam (unchanged, still `path_syntax_error` from a separate cause), and **bonus** otpop + maxmin (cold `model_infeasible`/`terminated` → MS 1, but **MISMATCH** — they have their own remaining stationarity bugs: otpop's #1393/#1335 [Days 6–7], maxmin's `stat_mindist` Case-b). otpop/maxmin are Solve-bucket transitions, **not genuine matches** — precise Solve/Match classification deferred to the Day-10 full retest.
**Cross-references:**
...
- **Traced Fix-Surface (Day-0):** **Candidate (structural localization, 2026-06-12; harness-residual confirmation pending Day 5).** Confirmed in `camshape_mcp.gms:421` `stat_r(i)`: the `lam_convexity(i-1)` cross-term is guarded by `$(ord(i) > 1)$(middle(i))` and `lam_convexity(i+1)` by `$(ord(i) <= card(i) - 1)$(middle(i))` — i.e. `middle(i)` AND the **looser** `ord` conditions, **not** the canonical `middle(i-1)`/`middle(i+1)` the hand-derived shape requires. This matches the prep hypothesis; the guard-construction site (`stationarity.py:1835` `_build_indexed_stationarity_expr`) is to be pinned by the instrumented trace + harness Case-b residual (≈ 396 at `stat_r('i1')`) on Day 5 (PR24).
