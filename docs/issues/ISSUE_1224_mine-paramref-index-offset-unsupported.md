...
- **Traced Fix-Surface (Day-0/Day-4):** **CONFIRMED via harness + instrumentation (2026-06-16).** Harness on mine → **Case b**, max-residual `stat_x(2,2,1)`. Instrumented the symbolic stationarity build: the cross-term was the single non-inverted `MultiplierRef(lam_pr, (k,l,i,j))`, missing the second var-ref entirely. **Build site = `src/kkt/stationarity.py`** (NOT the AD/Jacobian layer the prep doc guessed — PR24 correction): mine's per-`k` integer offsets get consolidated to zero by the launch-style Pattern-C path, so the var falls through to the non-inverted `else`. **Fix landed (Day 4):** new `_try_build_param_offset_crossterm` (tightly gated to a non-`Const` `IndexOffset` = mine only) builds the inverted cross-term directly from the body. Emit now matches the hand-derived shape exactly; blast radius = `mine_mcp.gms` `stat_x` line only.

### Day-4 outcome: stat_x fix landed; +1 Solve REPLAN'd → #1443 (Sprint 29)

The `stat_x` inversion (A) is **correct and landed**, but it is **necessary, not sufficient** — mine still does not reach MS 1. Empirically (Day 4): mine is a convex LP, yet the cold MCP is **MS 5 Locally Infeasible** with `x → 4e10` (despite `x.up=1`) and **49 INFES** across `comp_pr`/`comp_lo_x`/`comp_up_x`/`stat_x`/`def`. Root cause is the `pr(k,l+1,i,j)` **head-domain-offset** mis-emitting the broader KKT (a presolve dual-transfer misalignment — `lam_pr.l = abs(pr.m(k,l,i,j))` should read `pr.m` at `l+1` — plus a deeper cold-infeasible complementarity/bound coupling). The gate's "stat_x inversion → +1 Solve" hypothesis was **incomplete** (PR24: the harness + cold-solve revealed the deeper structure). Per PR25 the Day-4 result is **bucket-forward (stationarity-shape correction), not a genuine Solve gain**. The remaining head-domain-offset MCP correctness is **re-scoped to #1443 (Sprint 29)**; see `docs/issues/ISSUE_1443_*.md`.

- **Cross-links:** KNOWN_UNKNOWNS Category 1 (Unknowns 1.1, 1.2, 1.3); BASELINE_METRICS §2 mine provenance row (S27 +1 Translate; Solve REPLAN'd to #1443); successor issue **#1443**.
