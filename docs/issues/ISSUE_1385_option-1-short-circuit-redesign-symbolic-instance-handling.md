...
**Status:** **REPLAN → Sprint 32 (Sprint 31 Day 8, 2026-07-12) — a Day-8 finding ENLARGES the scope: the dominant blow-up is NOT the 1,152 constraint instances but the 369,024-instance 4-D `task(g,t,mn,mn)` variable's `stat_task` enumeration, which the 2-D constraint gate does nothing about.** Built the 2-D gate extension (`_is_blowup_2d_condition_equation`) empirically — it fires tightly on `tbal`/`equipb1`/`equipb2` and short-circuits their AD enumeration — **but sarf STILL times out**: `task` has 16·24·31·31 = **369,024** Cartesian instances (next-largest var = 48), so `stat_task` is enumerated 369K times regardless. So the design's fix-surface (2-D constraint gate + parametric cross-term) is **necessary but INSUFFICIENT** — the parametric emit must ALSO sparsify the 4-D `task` variable's stationarity (369K → the `$taskposs`-active subset), a strictly larger architectural undertaking than the already-4×-failed Sprint-26 symbolic-emit. Gate reverted (measurement-only; landing it would produce an incomplete MCP + no translate gain). sarf stays `translate_timeout`; +Translate deferred. Sprint targets already met (Match 95 ≥ 92, genuine floor 74 ≥ 73 at Day 5). See the Day-8 block in the Sprint-31 refresh below. _(was: PARTIALLY DONE Sprint 27 Day 7; REPLAN'd → Sprint 31 at Sprint 30 Day 9; → Sprint 30 at Sprint 29 Day 9; DEFERRED → Sprint 29; re-deferred at Sprint 28 Day 11.)_

> **🔴 DECIDED — REPLAN the atomic symbolic-emit to Sprint 31 (Sprint 30 Day 9, 2026-07-07).** Re-confirmed at Day 9: sarf still hits `translate_timeout` (>2 min; the 1-D `_is_blowup_dynamic_subset_equation` bails on `len(eq_domain) != 1`, so it never fires on sarf's 2-D `taskposs(g,t)`/`equipposs(m,t)` condition shape — `tbal(g,t)$taskposs(g,t)` [384], `equipb1(m,t)$equipposs(m,t)` [648], `equipb2(n,t)$equipposs(n,t)` [120]; 1,152 Cartesian instances). The **atomic** fix needs (a) a 2-D gate extension AND (b) a **new symbolic runtime-guard cross-term emit path** (`stationarity.py`) that differentiates each short-circuited body **once parametrically** in `(g,t,m,n)` — because the gate makes those equations enumerate **zero** instances, so the cross-terms cannot be assembled from per-instance Jacobian entries. **That symbolic-emit path is precisely the Sprint-26-Day-4 architecture that FAILED** (commit `243fe578`, reverted — `nu_slack("srn")`/`lam_demand("srn")` set-name-literal indices + dropped `J_gᵀ·lam` cross-terms). **Atomicity** (Unknown 3.1) forbids a safe partial: skip-only = an incomplete MCP (no Solve/Match); re-emit-without-correct-cross-terms = the wrong MCP that failed before. sarf is strictly harder than srpchase (4-D `task` × 5 guarded constraints × nested `taskposs`/`equipposs`). This is a **multi-day dedicated workstream, not a ~7h day** (matching Sprint-29's conclusion). **→ REPLAN to Sprint 31.** The hand-derived `stat_task` cross-terms (below) are the banked reference spec; sarf stays `translate_timeout` this sprint (no +Translate). The **translate-only 2-D-gate partial** (srpchase-style — sarf translates by skipping the 3 constraints, no cross-terms) was considered and **declined**: it lands an incomplete MCP that ISSUE_1385's atomicity rejects, and the Day-9 goal is the *atomic* fix — so we defer whole rather than bank a partial. _(was: DEFERRED → Sprint 29; re-deferred at Sprint 28 Day 11 — the day was consumed by the camcge Task-6 gate + the #1374/#1400 cleanups.)_
...
**Date:** 2026-05-12
**Last Updated:** 2026-07-12 (Sprint 31 Day 8 — REPLAN → Sprint 32 with an ENLARGED scope: the 2-D gate fires but sarf still times out on the 369,024-instance 4-D `task` variable stationarity, not the 1,152 constraints; prior: 2026-07-07 Sprint 30 Day 9 — REPLAN the atomic symbolic-emit → Sprint 31)
...
> **🔄 Sprint-31 refresh (Prep Task 6, 2026-07-09): PROCEED — the tractability gate is O(constraints), not O(instances).** The symbolic runtime-guard re-emit must differentiate each short-circuited body **once parametrically** in `(g,t,m,n)` — O(constraints), not sarf's **1,152** Cartesian instances (the Option-1 short-circuit's whole point). **PROCEED gate:** time `sarf_mcp.gms` against the translate budget (sub-timeout); the re-emitted `stat_task` matches the banked 6-guarded-term hand-derivation with **no set-name-literal multiplier indices** (the Sprint-26-Day-4 `nu_slack("srn")` failure mode); the re-emit + `J_gᵀ·lam` cross-terms land **atomically**; the golden is byte-stable. **REPLAN** (re-scope) if the parametric emit re-triggers the translate timeout. Fix surface pinned by Prep Task 9. See `docs/planning/EPIC_4/SPRINT_31/PHASE_0_ACCEPTANCE_GATES.md` §P4.
>
> **➡️ Day-8 OUTCOME (2026-07-12): REPLAN → Sprint 32 — the tractability gate FAILED for a newly-found reason.** Built the 2-D gate (`_is_blowup_2d_condition_equation`): it fires tightly on `tbal`/`equipb1`/`equipb2` (verified) and short-circuits their AD (the warnings confirm "skipping AD enumeration") — **but sarf STILL times out.** The dominant blow-up is the **4-D `task(g,t,mn,mn)` variable: 16·24·31·31 = 369,024 instances** (next-largest var = `xcrop` at 48), whose `stat_task` is enumerated 369K times regardless of the constraint short-circuit. **So the design premise (blow-up = the 1,152 constraint instances; fix = 2-D gate + parametric cross-term) is INCOMPLETE** — the parametric emit must ALSO sparsify the `task` variable's stationarity to the `$taskposs(g,t)`-active subset (the banked `stat_task(g,t,m,n)$taskposs(g,t)` shape IS parametric, but the current emit enumerates the full 369K before guarding). This is a **strictly larger** undertaking than the 4×-failed Sprint-26 architecture. Gate REVERTED (measurement-only: it neither makes sarf translate nor can land — atomicity + insufficiency). **The enlarged scope for the Sprint-32/Epic workstream: parametric stationarity emit for a 369K-instance 4-D variable + the parametric constraint cross-terms, atomically.** No `src/` landed.
>
