...
> **🟠 Day-6 diagnosis (Sprint 29, 2026-06-29) — distributed multi-site head-offset; leans REPLAN (Day-7 decision).** Confirmed harness Case b `stat_x(4,1,1)` 1.33, dual-transfer CONSISTENT. **Cold solve = MS5, 49 INFES + 10 REDEF.** Mapped the 49 INFES by row-type: the `comp_pr` precedence rows by direction `k` (nw 6 / ne 9 / se 12 / sw 11 = 38) + the `def` objective row + bound rows — **all from the `pr(k,l+1,i,j)$c(l,i,j)` head-domain-offset** (IR: `pr.has_head_domain_offset=True`, domain stored as the base `(k,l,i,j)`, body `x(l,i+li,j+lj) ⊥ x(l+1,i,j)`). `x.up=1` is emitted (x bounded [0,1]) so the 4.07e10 is the **comp_pr LCP residual**, not an x value — the precedence complementarity is internally inconsistent.
