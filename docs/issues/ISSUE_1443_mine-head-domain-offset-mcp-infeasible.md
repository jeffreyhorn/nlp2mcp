...
**GitHub:** #1443
**Status:** **Sprint 30 Day 6 (2026-07-07): REPLAN → Sprint 31** (head-domain-offset emit **architecture** workstream — needs a foundational IR change before the coordinated fix is even expressible; see the Day-6 decision block below). _(was: Sprint 29 Day 7 REPLAN → Sprint 30)_
...
> **🔄 Sprint-31 refresh (Prep Task 6, 2026-07-09): PROCEED — the head-offset IR plumbing is a favorable field addition; the gate is the round-trip fixture + the cold-INFES histogram.** Prep Task 3 (`docs/planning/EPIC_4/SPRINT_31/HEAD_OFFSET_IR_PLUMBING_DESIGN.md`) established the head offset is discarded at **parse** (not normalize), so the fix is a field addition on `EquationDef` (`head_domain_offsets`, mirroring `declaration_domain`) + copy-through at the ~3 reconstructor sites — **NOT a deep normalize rewrite**. **PROCEED gate:** the round-trip unit reproduction (`tests/fixtures/head_offset_ir_roundtrip.gms`) green **before** the emit change (Phase 1); then the shared 3-site helper drives all four k-directions → cold **MS-1** (Phase 2). **REPLAN** to a Sprint-32 head-offset-Phase-3 workstream if a 4th bound-complementarity site persists after the `comp_pr` fix. See `docs/planning/EPIC_4/SPRINT_31/PHASE_0_ACCEPTANCE_GATES.md` §P1.
>
> **✅ Phase 1 LANDED (Sprint 31 Day 1, 2026-07-09) — the IR plumbing is in; the field is inert.** Added `EquationDef.head_domain_offsets` (per-position `IndexOffset | None` tuple aligned to `domain`, mirroring `declaration_domain`); `has_head_domain_offset` is now derived from it. Producer `_domain_list_head_offsets`/`_index_list_head_offsets` in `src/ir/parser.py` (reuse `_process_index_expr`; circular offsets excluded; filtered in lockstep with literal-domain stripping); copy-through at the 3 reconstructor sites (`sqr_reformulation.py` ×2, `complementarity.py:242`); `normalize` passthrough. **Phase-1 gate MET:** round-trip fixture green (`tests/unit/ir/test_head_domain_offsets.py`, 7 assertions) + the 5 head-offset models (mine/robert/camshape/ramsey/abel) emit **byte-identical** to committed goldens (zero emit change — Unknown 1.4 ✅). **Phase 2 (Days 2–3): the shared 3-site helper reads `head_domain_offsets` from the `comp_pr` source `EquationDef` → cold MS-1.**
>
> **✅ Phase 2 Day 2 LANDED + a decisive re-diagnosis (Sprint 31 Day 2, 2026-07-10).** An empirical warm-start experiment **overturns the "3 identical maps" premise**: `comp_pr` (Site 1) and `stat_x` (Site 3) are **already correct** — warm-starting the emitted MCP from the NLP optimum with the head-shifted dual transfer reaches **MODEL STATUS 1 Optimal, profit 17500 = the LP optimum** (direct LP solve also 17500). So the NLP KKT point **is** an exact LCP solution of the current emit. The two remaining blockers:
> 1. **Site 2 (the `--nlp-presolve` dual transfer) — FIXED this day.** The NLP labels the equation instance (and stores `pr.m`) at the **shifted head label** `(k,l+1,i,j)` while `lam_pr` is paired at the base `(k,l,i,j)`; the transfer read `pr.m(k,l,i,j)` (base) — the wrong instance. New shared helper `head_offset_marginal_index_map` (`emit_gams.py`) reads `head_domain_offsets` (Phase-1 field) and shifts the read to `pr.m(k,l+1,i,j)`. Blast radius **zero** on all 13 committed presolve goldens + all cold goldens (byte-identical); mine's presolve transfer is now shifted. Sites 1 & 3 realise the same head↔base correspondence inline (verified consistent) — the helper centralises Site 2's read.
> 2. **The "4th site" (Day-3 gate) — the non-`d` bound-fixing.** With the shifted transfer, mine's presolve MCP is **still MS-5 (22058)** *until* the `x.fx(l,i,j)$(not d(l,i,j)) = 0` fixing is relaxed: hard-fixing the inactive (non-`d`) primal instances to 0 makes the LCP infeasible even from the exact NLP KKT warm point; leaving them free (`x.up = inf`, bound via `comp_up_x`) → **MS-1, 17500**. This is exactly the bound-complementarity 4th site the design §6 flagged. Candidate fix exists (relax/scope the non-`d` fixing for head-offset models) but the mechanism is subtle → **Day 3 close-or-REPLAN gate.**
>
