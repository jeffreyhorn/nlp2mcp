...
class _ModelBuilder:
    ...
    def _handle_table_block(self, node: Tree) -> None:
        ...
        Grammar: table_block: "Table"i ID "(" id_list ")" STRING? table_row+ SEMI
        ...
        First table_row is the column headers (j1, j2)
        Subsequent table_rows are data rows (i1 1 2, i2 3 4)
        """
        # Extract name and domain
        name = _token_text(node.children[0])
        domain = _id_list(node.children[1])

        # Find all table_row nodes
        table_rows = [
            child
            for child in node.children
        ...
        if not table_rows:
            # Empty table - create parameter with empty values
            self.model.add_param(ParameterDef(name=name, domain=domain, values={}))
            return

        # First row contains column headers
        col_headers = [
            _token_text(child) for child in table_rows[0].children if isinstance(child, Token)
        ]

        # Subsequent rows contain data
        values = {}
        for row in table_rows[1:]:
            row_tokens = [child for child in row.children if isinstance(child, Token)]
            if not row_tokens:
                continue

            # First token is the row header
            row_header = _token_text(row_tokens[0])

            # Remaining tokens are values (one per column)
            for col_idx, value_token in enumerate(row_tokens[1:]):
                if col_idx < len(col_headers):
                    col_header = col_headers[col_idx]
                    value_text = _token_text(value_token)

                    # Try to parse as float, default to 0.0 if empty or non-numeric
                    try:
                        value = float(value_text)
                    except ValueError:
                        value = 0.0

                    # Create tuple key from row and column headers
    ...
    def _handle_table_block_old(self, node: Tree) -> None:
        """OLD VERSION - keeping for reference, replaced by new version below"""
        pass
