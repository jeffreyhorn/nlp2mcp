...
def _build_sameas_guard(
    ...
    """Build a sameas guard for multiplier terms referencing partial variable instances.

    Issue #767 / #764 / #1049: When a constraint (scalar or indexed) references
    only a subset of an indexed variable's instances, the multiplier term must
    be guarded so that it only appears for the relevant instances in the
    stationarity equation.  Used from both the scalar-constraint path and the
    indexed-constraint path (via ``group_entries`` in
    ...
    for _eq_name, entries in constraint_entries.items():
        ...
        if eq_indices:
            ...
            if mult_domain:
                ...
                # entry offset groups.  Structural offsets can legitimately
                # appear in only a small number of rows (including boundary-
                # only lead/lag terms), so do not broadly prune low-coverage
                # groups.  Instead, only drop the specific degenerate case:
                # many singleton-coverage groups that all originate from the
                # same equation row.
                if len(offset_groups) > 2:
                    rows_by_group = {
                        key: {r for r, _ in group_entries}
                    ...
                    if len(singleton_groups) >= 2 and len(singleton_rows) == 1:
                        filtered = {
                            k: v for k, v in offset_groups.items() if k not in singleton_groups
                        }
                        if filtered:
