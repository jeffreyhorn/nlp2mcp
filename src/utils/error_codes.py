...
def format_convexity_warning(
    ...
    """
    Format a convexity warning with error code and documentation link.

    Args:
        code: Error code (e.g., "W301")
        equation: Equation name where pattern was found
        message: Description of the issue
        details: Optional additional details

    Returns:
        Formatted warning message with documentation link

    Example:
        >>> format_convexity_warning("W301", "circ_eq", "Nonlinear equality may be nonconvex")
        Warning W301: Nonlinear equality may be nonconvex
          Equation: circ_eq

          This pattern may indicate a nonconvex optimization problem.
          Docs: https://github.com/...#w301-nonlinear-equality
    """
    error_info = get_error_info(code)
    if not error_info:
        # Fallback if code not found
        return f"Warning {code}: {message} in {equation}"

    result = f"{error_info.level} {code}: {error_info.title}\n"
    result += f"  Equation: {equation}\n"

    if details:
        result += f"  Details: {details}\n"

    result += f"\n  {message}\n"
    result += f"  Docs: {error_info.doc_url()}\n"
