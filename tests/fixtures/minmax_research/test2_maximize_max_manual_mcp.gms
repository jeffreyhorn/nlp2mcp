* Test Case 2 Manual MCP: maximize z where z = max(x, y)
* Strategy 2 reformulation: z >= x, z >= y
* For maximization, objective gradient is -1 (maximizing = minimizing negative)
...
Equations
    ...
    ineq_x_le_z  inequality x <= z (reformulated from z >= x)
    ineq_y_le_z  inequality y <= z (reformulated from z >= y)
...
Display "Expected: z = 20, x = 10, y = 20, lam_y > 0, lam_x = 0";
