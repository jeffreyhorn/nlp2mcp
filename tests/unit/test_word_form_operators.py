"""Unit tests for word-form comparison operators (ne, eq, lt, gt, le, ge).
...
class TestWordFormOperators:
    """Test that word-form comparison operators parse correctly."""
...
"""
        ...
        # The expression should contain a Binary with op="<>"
        assert len(model.params["y"].expressions) > 0
        _key, expr = model.params["y"].expressions[0]
        # Find the comparison operator in the expression tree
