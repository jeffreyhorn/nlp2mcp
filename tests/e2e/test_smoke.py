...
@pytest.mark.e2e
class TestGAMSEmitterSmoke:
    """Smoke tests for GAMS MCP emission."""

    def test_gams_emitter_smoke(self, manual_index_mapping):
        """Basic GAMS emission smoke test.

        Verifies that a KKT system can be emitted to GAMS MCP format
        without crashing and produces valid-looking output.
        """
        model = ModelIR()
        model.objective = ObjectiveIR(sense=ObjSense.MIN, objvar="obj")
        ...
        assert "Generated by nlp2mcp" in output
        assert "Variables" in output
        assert "Equations" in output
        assert "Model mcp_model" in output
        assert "Solve mcp_model using MCP" in output

        # Verify original symbols not lost
        assert "obj" in output
        assert "x" in output

        # Verify variable kinds preserved (x should be in Positive Variables)
        assert "Positive Variables" in output

        # Verify stationarity equation for x (not obj)
        assert "stat_x" in output
        assert "stat_obj" not in output

        # Verify objective equation paired with obj
        assert "objdef" in output

    def test_gams_emitter_with_comments(self, manual_index_mapping):
        """Test GAMS emission with explanatory comments."""
        model = ModelIR()
        model.objective = ObjectiveIR(sense=ObjSense.MIN, objvar="obj")
        ...
        # Emit with comments
        output = emit_gams_mcp(kkt, add_comments=True)

        # Verify comments present
        assert "$onText" in output
        assert "$offText" in output
        assert "KKT System Components" in output
        assert "Stationarity" in output
        assert "Complementarity" in output

    def test_gams_emitter_without_comments(self, manual_index_mapping):
        """Test GAMS emission without comments."""
        model = ModelIR()
        model.objective = ObjectiveIR(sense=ObjSense.MIN, objvar="obj")
